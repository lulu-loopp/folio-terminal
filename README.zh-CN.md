...
- 窗口能容纳的页面数量与其预览窗格数量相同。第二个页面会在独立窗格中打开，而不是让第一个窗格转到新地址，因此可以锁定一个页面，再在其旁边打开另一个。
...
- 窗格放到两个标签页之间的接缝上时，会成为二者*之间*的新标签页，接缝上会画出一条线标示落点；放到标签页自身上则加入该标签页的布局。横向标签条、竖向标签栏与卡片列都以相同方式识别接缝。
...
从未运行过 Folio 的机器只会看到一张卡，仅此一次，名为**首次配置**。它把所有涉及写入 `%APPDATA%\Folio` 之外内容的设置一并询问：是否每天检查新版本、是否把**在 Folio 中打开**加入资源管理器右键菜单、是否安装 PowerShell 整合，以及本机实际安装的 Claude Code、Codex 和 Copilot CLI 各自对应的一行选项。主题、字体、字号、语言或布局等均不在其中——这些设置距离一次点击只有一步，且设置不当的那段时间里也不会造成任何代价。

<picture>
  <source media="(prefers-color-scheme: dark)"
          srcset="docs/screenshots/first-run-dark.png">
  <img src="docs/screenshots/first-run-light.png" width="100%"
       alt="首次配置卡位于刚启动的窗口中央。「每天检查新版本」处于开启状态；「从资源管理器用 Folio 打开」与「安装 PowerShell 整合」处于关闭状态，各有一行说明文字。标题「本机检测到的 agent」下方有 Claude Code 与 Codex 两行，均未开启，各自标明其开关写入的文件。底部有一行说明卡上的每一行在设置里同样存在，然后是「打开设置」、「暂不」与「完成」三个按钮。卡后方，有一个标签页与一个提示符。">
</picture>

**该卡上的每一行同样出现在设置中**，因此卡面上的选择并非最后机会。**完成**应用已开启的选项；**暂不**或按 `Esc` 保持默认值并关闭卡——即更新检查开启，其余选项关闭——且不更改任何内容。无论选择哪种方式，卡都不会再次出现，而其背后的 shell 在整个过程中一直在运行。如果在此版本之前已在使用 Folio，则看不到此卡：`settings.json` 已记录了相关信息。

第一个标签页打开本机实际拥有的第一个 shell。五条内置配置按顺序查找——PowerShell 7、Windows PowerShell、WSL、Git Bash、命令提示符——程序未安装的配置不会出现在启动 shell 的菜单中，但仍保留在设置中的配置文件页，呈灰色并标明所查找的程序名称。七条 agent 配置以相同方式在 Windows PATH 中查找，这与卡上 agent 行的查找方式相同。

PowerShell 整合向 PowerShell 自行命名的 `$PROFILE` 添加一行——`. "$env:APPDATA\Folio\shell-integration\folio.ps1"`——在把原有文件按现状复制为带日期的备份之后；删除该行即可撤销。该文件的位置由 shell 决定，因此卡应答时仍保持开启的行会在下一个启动的 PowerShell 中生效，设置 > 终端会持续显示此状态直至生效。若未在卡上收到询问，当某个 PowerShell 窗格首次输出内容时，提示条提供同样的选项：**加进 `$PROFILE`** 执行操作，**不再提示**终止询问，而关闭提示条不作出任何决定，因此下一个 PowerShell 会再次询问。命令标记和行内 `$…$` 公式依赖该整合运行。Git Bash 和 WSL 无需此项，也不会在磁盘上留下任何内容。

设置的 Agent 页上的三行**默认不安装任何内容**，且它们并非恰好关闭的默认选项：每行读取工具自身的配置文件并报告其中内容。在新机器上，三个文件均不存在，因此全部显示为关闭。
