...
从未运行过 Folio 的机器会得到一张卡，只出现一次，标题为**首次配置**。它把所有会在 `%APPDATA%\Folio` 之外写入内容的问题放在一起问：是否每天检查新版本、是否把「在 Folio 中打开」放进资源管理器的右键菜单、是否安装 PowerShell 整合，以及本机实际装有的 Claude Code、Codex、Copilot CLI 各一行。不问主题、字体、字号、语言与布局——那些一个点击即可改回，错着的时候也没有代价。

**卡上的每一项也都是设置里的一行**，所以它上面没有任何一次性的机会。**完成**兑现开着的行；**暂不** 与 `Esc` 按出厂值关闭——更新检查开、其余关——什么也不改。两种选择下卡都不会再出现，而它后面的 shell 一直在跑。在这个版本之前已在使用 Folio 的用户不会看到它：`settings.json` 已经记下了这件事。

第一个标签页打开系统中排在最前的 shell。五条内置 shell 配置按 PowerShell 7、Windows PowerShell、WSL、Git Bash、命令提示符的顺序查找；未安装对应程序的配置不会出现在用于启动 shell 的菜单中，而是保留在设置的配置文件页，在那里显示为灰色，并写明所查找的程序名称。七条 agent 配置以同样方式在 Windows PATH 中查找，卡上的 agent 行用的就是这一次查找。

PowerShell 整合在 PowerShell 自己报出的 `$PROFILE` 末尾追加一行 `. "$env:APPDATA\Folio\shell-integration\folio.ps1"`，追加前先将原文件按日期另存为备份；删除该行可撤销。该文件在哪里由 shell 得知，因此卡上这一行开着按下完成时，改动在下次启动 PowerShell 时生效，在那之前设置 > 终端会如实说明。若卡上未问过这一项，第一个 PowerShell 窗格输出内容后会显示提示条：**加进 `$PROFILE`** 完成追加，**不再提示** 终止询问；直接关闭提示条不构成决定，下一个 PowerShell 窗格会再次询问。命令标记与行内 `$…$` 公式依赖该整合。Git Bash 与 WSL 无需安装任何内容，亦不在磁盘上留下痕迹。
