...
- **PSReadLine。** Windows PowerShell 5.1 自带的还是 PSReadLine 2.0.0，它的编辑锚点取自窗口改变大小之前数出的格数，所以把窗口拖窄，输入行会画回原来的位置，盖住已经移走的文字。Folio 内置了一份打过补丁的 2.4.6，你让它装，它就装进你的模块目录。
...
Folio 不向任何地方发送数据：没有遥测、没有统计、没有崩溃上报、没有更新检查，自己也没有网络客户端。唯一联网的是你在网页预览里打开的那个页面。
...
- **在手机上看谁在等你。** agent 仍然跑在电脑上，手机负责显示哪个在等你回话，并且能回它一句。
