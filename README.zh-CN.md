...
第一个标签页打开系统中排在最前的 shell。五条内置 shell 配置按 PowerShell 7、Windows PowerShell、WSL、Git Bash、命令提示符的顺序查找；未安装对应程序的配置在选择器中显示为灰色而非隐藏。七条 agent 配置以同样方式在 Windows PATH 中查找，卡上的 agent 行用的就是这一次查找。
