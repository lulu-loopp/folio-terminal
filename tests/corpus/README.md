...
repository path until they were scrubbed on 2026-08-27. `tests/corpus/PROVENANCE.md` lists
source, licence and scrub date for every file here, and the classes of identity that were
replaced. The substitutions are length-preserving, so replay feeds the same byte counts
through the same length prefixes and the same column alignment as the original capture.
