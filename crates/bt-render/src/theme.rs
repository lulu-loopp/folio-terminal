...
/// owner ruling 2026-09-12 「就药丸」).
///
/// A tab on a window whose title bar is the platform's does not join the pane:
/// it is a 30pt pill centred on the 40px strip, so the strip's floor runs
/// unbroken under it. Thirty and not thirty-four because the shape has air on
/// both sides now rather than on one — the four points the attached tab spent
/// on the 6px of air above it buy five above and five below.
///
/// Only the silhouette and the vertical placement change. Everything a tab is
/// made of — padding, mark, gap, font, the `×`, the pin, the badge, the ring,
/// the width tiers — is the same number it is on an attached tab, which is why
/// there is one constant here and not a second table.
pub const WINDOW_TAB_FLOAT_HEIGHT_LOGICAL_PX: f32 = 30.0;
/// **How far the first floating pill stands from the run's left edge**
/// (`mock-mac-strict.html` `--after-lights: 12px`, owner-specified).
///
/// An attached run leads in by `WINDOW_TAB_RADIUS_LOGICAL_PX`, and that is not
/// air: `.tabs-inline { padding-left: var(--tabr) }` exists so the first tab's
/// outward skirt has somewhere to land, which is why the silhouette still
/// starts on the window's own edge. A pill has no skirt, so its lead-in is air
