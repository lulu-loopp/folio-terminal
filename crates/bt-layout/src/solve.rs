...
impl AxisPlan {
    ...
    fn leaf_demand(&self, seat: &Seat, metrics: &SeatMetrics) -> LogicalPx {
        ...
        match seat.fixed_width(metrics, self.axis) {
            Some(full) => shrink_fixed(
                full,
                metrics.min_size(seat.kind, self.axis),
                self.fixed_shrink_ppm,
            ),
            None => metrics.min_size(seat.kind, self.axis),
        }
    }
        ...
        match node {
            LayoutNode::Seat(seat) => {
                if self.is_collapsed(seat.id) {
                    ...
                    seat.fixed_width(metrics, self.axis).map(|full| {
                        shrink_fixed(
                            full,
                            metrics.min_size(seat.kind, self.axis),
                            self.fixed_shrink_ppm,
...
fn least_sufficient_shrink(
    ...
    let fits = |t: u32| {
        ...
        probe.subtree_demand(tree, metrics) <= avail
    };
...
fn plan_axis(
    ...
    // minimum and every fixed column at its floor. L3 collapses non-focus seats,
    // farthest from the focus first, and stops the moment the tree fits.
...
pub fn solve(
    ...
    match mode {
        LayoutMode::Focus { stage } => solve_focused(tree, viewport, metrics, stage),
        LayoutMode::Parallel => solve_parallel(tree, viewport, metrics, focus),
    }
}
...
fn solve_parallel(
    ...
    let row = plan_axis(tree, Axis::Row, viewport.extent(Axis::Row), metrics, focus)?;
    let col = plan_axis(tree, Axis::Col, viewport.extent(Axis::Col), metrics, focus)?;
        ...
        LayoutNode::Split {
            ...
            let (rect_a, rect_b) = match dir {
                Axis::Row => (
                    ...
                    LogicalRect::new(cut + DIVIDER, rect.top, rect.right, rect.bottom),
                ),
                Axis::Col => (
                    LogicalRect::new(rect.left, rect.top, rect.right, cut),
                    LogicalRect::new(rect.left, cut + DIVIDER, rect.right, rect.bottom),
                ),
            };
