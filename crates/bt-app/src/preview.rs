...
/// Which icon a file's name earns (file-type icons, 2026-08-15).
///
/// Six classes where [`PreviewFtype`] has five, and the difference is the point:
/// this question is asked by *drawings* and that one is asked by *readers*, so
/// the two agree about everything except the one place a reader has no opinion.
/// `preview_ftype` puts `main.rs` and `Cargo.toml` in one class because it opens
/// both the same way; an icon has something to say about the difference.
///
/// **It is a refinement of that table and never a second copy of it.**
/// [`file_icon_class`] begins by asking `preview_ftype` and only subdivides the
/// answer, so the day an extension moves between reader classes its icon follows
/// without anybody remembering to edit a list here — and the day someone adds an
/// extension here that the reader has never heard of, the arm that would read it
/// is unreachable, which `the_refinements_are_subsets_of_the_ftype_table_and_disjoint`
/// makes a failing test rather than a silent nothing.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum IconClass {
    Image,
    Markdown,
    /// Source: something a person writes in a language a machine runs.
    Code,
    /// Configuration and data: something a person writes for a machine to read.
    Config,
    Table,
    /// A plain document — **and** every name this build cannot type.
    ///
    /// That the neutral and the fallback are one class is a decision and not a
    /// shortcut. A badge is a *claim* about a file, and the honest claim about a
    /// `.zip` is none: badging the fallback with the ruled lines that mean prose
    /// would draw an archive as an essay. So this class alone wears the unbadged
    /// page — [`crate::marks::ChromeMark::File`], the icon every file in the
    /// product wore before this pass.
    Document,
}

// **A class has no colour, and there is no `ink` method to ask for one** (user
// ruling, 2026-08-15). This enum shipped for a few hours with a companion set of
// five palette hues and an `IconClass::ink` to pick between them; the ruling
// struck them out on sight of the real window, because six coloured rows in a
// column is a shelf and this product spends exactly one accent. A page therefore
// takes whatever ink the surface it stands on already spent on a file — which is
// not a decision this module makes or needs to know about — and everything a
// class has to say, it says with the badge in [`IconClass::mark`].

/// Extensions inside [`TEXT_EXTENSIONS`] that name source code.
///
/// **A refinement, not a table of its own**: every entry here must also be in
/// `TEXT_EXTENSIONS`, because this list is only ever consulted after
/// `preview_ftype` has already answered [`PreviewFtype::Text`]. A language this
/// window cannot open — `.go`, `.c` — is `Unknown` there and a plain page here,
/// which is the consistent answer: the icon says what the window can do with the
/// file, and about those it can do nothing.
const CODE_EXTENSIONS: [&str; 5] = ["rs", "py", "js", "ts", "html"];

/// Extensions inside [`TEXT_EXTENSIONS`] that name configuration or data.
///
/// The same refinement rule as [`CODE_EXTENSIONS`], and the two are disjoint by
/// test. `lock` is here rather than with code because a lock file is generated
/// data that nobody writes by hand — it is the most machine-facing thing in the
/// tree.
///
/// `.gitignore` is deliberately absent, and so is every other dotfile: they
/// reach this function as [`PreviewFtype::Text`] by the *name* rule rather than
/// by an extension, and a rule about names cannot say which of them is config —
/// `.env` is, `.bashrc` is arguable, `.mailmap` is not.
const CONFIG_EXTENSIONS: [&str; 5] = ["json", "toml", "yml", "yaml", "lock"];

/// Which of the six icon classes a file name belongs to.
pub fn file_icon_class(name: &str) -> IconClass {
    match preview_ftype(name) {
        PreviewFtype::Image => IconClass::Image,
        PreviewFtype::Markdown => IconClass::Markdown,
        PreviewFtype::Table => IconClass::Table,
        PreviewFtype::Unknown => IconClass::Document,
        // The one class a reader lumps together and a drawing does not.
        PreviewFtype::Text => {
            let ext = extension_of(name);
...
impl IconClass {
    /// The mark this class is drawn as.
    #[must_use]
    pub fn mark(self) -> crate::marks::ChromeMark {
        use crate::marks::ChromeMark;
        match self {
            Self::Image => ChromeMark::FileImage,
            Self::Markdown => ChromeMark::FileMarkdown,
            Self::Code => ChromeMark::FileCode,
            Self::Config => ChromeMark::FileConfig,
            Self::Table => ChromeMark::FileTable,
            Self::Document => ChromeMark::File,
        }
    }
}

/// The class a name earns, or the neutral one when the caller has no name.
///
/// The two-line dance every consumer of [`file_icon_class`] would otherwise
/// write for itself: a pane head with no document open, a drag payload with no
/// name, a collapsed bar whose caption fell back to a path — all of them hold an
/// `Option` and all of them owe the same answer to `None`.
#[must_use]
pub fn file_icon_class_of(name: Option<&str>) -> IconClass {
    name.map_or(IconClass::Document, file_icon_class)
}
...
mod tests {
    ...
    /// ③b The icon classifier, class by class (file-type icons, 2026-08-15).
    ///
    /// Mutation: return [`IconClass::Document`] unconditionally, or move `"rs"`
    /// from [`CODE_EXTENSIONS`] to [`CONFIG_EXTENSIONS`], or drop the
    /// `PreviewFtype::Text` arm's refinement so every readable name is a plain
    /// document again.
    #[test]
    fn the_icon_classifier_answers_each_class() {
        for name in ["a.png", "a.jpg", "a.jpeg", "a.svg", "a.gif", "a.webp"] {
            assert_eq!(file_icon_class(name), IconClass::Image, "{name}");
        }
        assert_eq!(file_icon_class("README.md"), IconClass::Markdown);
        assert_eq!(file_icon_class("cases.csv"), IconClass::Table);
        for name in ["a.rs", "a.py", "a.js", "a.ts", "a.html"] {
            assert_eq!(file_icon_class(name), IconClass::Code, "{name}");
        }
        for name in ["a.json", "a.toml", "a.yml", "a.yaml", "Cargo.lock"] {
            assert_eq!(file_icon_class(name), IconClass::Config, "{name}");
        }
        // Readable, but nothing further to say: prose, a patch, and the two
        // names that only *look* like config.
        for name in ["a.txt", "a.diff", "a.patch", "a.gitignore"] {
            assert_eq!(file_icon_class(name), IconClass::Document, "{name}");
        }
        // A dotfile is a plain document by the same reading `preview_ftype`
        // gives it: it is text by convention, and convention says nothing about
        // what *kind* of text.
        assert_eq!(file_icon_class(".gitignore"), IconClass::Document);
        assert_eq!(file_icon_class(".env"), IconClass::Document);
        // A name this build cannot read at all is a plain document too — the
        // page with no claim on it. It is the fallback and the neutral, and
        // those are deliberately the same thing.
        for name in ["a.exe", "a.dll", "a", "a.zip", "a.PNG.zip"] {
            assert_eq!(file_icon_class(name), IconClass::Document, "{name}");
        }
        // Case-insensitive, because the table it consumes is.
        assert_eq!(file_icon_class("A.PNG"), IconClass::Image);
        assert_eq!(file_icon_class("MAIN.RS"), IconClass::Code);
        assert_eq!(file_icon_class("CARGO.TOML"), IconClass::Config);
    }

    /// ③b′ A **caption is not a name**, and only a name may be classified.
    ///
    /// The regression that made [`crate::seats::ChromeContent::preview_documents`]
    /// exist, and it was found on the real machine rather than here: a picture's
    /// head prints `sunset.png — 800×600`, that string was handed to the
    /// classifier as if it were a file name, and the last dot in it is followed
    /// by `png — 800×600` — an extension nobody has registered. The picture drew
    /// the plain page while the tree row beside it drew the right one.
    ///
    /// This test does not fix that; the two separate lookups do. What it pins is
    /// the *reason* they are separate, so the day somebody folds them back into
    /// one string the argument is on the record.
    ///
    /// Mutation: teach `extension_of` to stop at the first space, which "fixes"
    /// this case — and the assertion still stands, because it asserts the
    /// classifier's honest refusal rather than a workaround for a caller that
    /// handed it the wrong thing.
    #[test]
    fn a_caption_is_not_a_name_and_does_not_classify_as_one() {
        assert_eq!(file_icon_class("sunset.png"), IconClass::Image);
        assert_eq!(
            file_icon_class("sunset.png \u{2014} 800\u{d7}600"),
            IconClass::Document,
            "a caption carries what the head has room to add, and a name does \
             not — so the caption is not a name and must not be classified"
        );
    }

    /// ③c The two refinement tables live *inside* the ftype table's `Text`
    /// class, and nowhere else.
    ///
    /// This is the whole of what keeps one extension list from becoming three.
    /// An extension named here but absent from [`TEXT_EXTENSIONS`] is
    /// unreachable — `file_icon_class` never enters the arm that would read it —
    /// and an extension named in both refinements is a coin toss decided by
    /// which `if` was written first.
    ///
    /// Mutation: add `"go"` to [`CODE_EXTENSIONS`], or `"json"` to both.
    #[test]
    fn the_refinements_are_subsets_of_the_ftype_table_and_disjoint() {
        for ext in CODE_EXTENSIONS.iter().chain(CONFIG_EXTENSIONS.iter()) {
            assert!(
                TEXT_EXTENSIONS.contains(ext),
                "`{ext}` refines a class `preview_ftype` never puts it in, so it \
        ...
        for ext in CODE_EXTENSIONS {
            assert!(
                ...
                "`{ext}` is claimed by both refinements"
            );
        }
    }
