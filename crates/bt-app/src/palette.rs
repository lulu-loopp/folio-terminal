...
//!
//! DESIGN.md §7.54. The surface is `design/ui-mockup.html` 2363-2394 (its style),
...
/// what makes the empty-query shape (DESIGN.md §7.54 ②) a case of the same code
...
/// are always drawn**.
///
/// **They are sections and they never interleave** (DESIGN.md §7.54 ②). One
...
/// DESIGN.md §7.54 ② — the cap on `Actions` and `Settings`.
const SHORT_SECTION_CAP: usize = 6;
/// DESIGN.md §7.54 ② — the cap on `Places`, `Commands` and `Files`.
...
mod list_tests {
    ...
    ///
    /// This is the ruling the whole shape rests on (DESIGN.md §7.54 ②): a query
    ...
    ///
    /// Two halves, and each is a decision (DESIGN.md §7.54 ②). Only `Actions`
