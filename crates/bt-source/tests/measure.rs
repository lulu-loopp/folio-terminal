...
fn the_index_costs_what_the_plan_budgeted() {
    ...
    // Cold: this process has never lowered anything, and `build` does not look
    // in the cache.
...
/// rather than assumed.
///
/// One process per compiled test target — a package's library, each of its
/// binaries, each of its integration tests — because each is its own executable
/// and each would build its own index. Doc tests are not counted: a doc test is
/// not where a source guard lives. An invocation carrying `--no-run` compiles
/// and starts nothing, so it is not here either.
