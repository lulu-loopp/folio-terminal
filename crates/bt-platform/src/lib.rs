...
mod quiet_door_tests {
    ...
    /// stops being one.
    /// **The one child this workspace builds that is not a door**, and why it
    /// is not one.
    ///
    /// `bt-pty`'s Unix suite asks the operating system's own process table
    /// whether a child it reaped is really gone, and it asks `ps` because
    /// asking `waitpid` again would be asking this crate's bookkeeping whether
    /// this crate's bookkeeping is right. It is `#[cfg(unix)]` and it is a
    /// test, so the console this gate exists to refuse is a console it can
    /// never be handed: Windows does not compile the line, and Unix has no
    /// console to flash.
    ///
    /// Named as one exact occurrence rather than as a file, so that a *second*
    /// child built in that file — or that one turned into something else —
    /// fails the gate the way any other would. Arrived with M1-6, which is the
    /// slice that gave `bt-pty` real Unix children.
    const PROCESS_TABLE_READER: (&str, &str) = (
        "crates/bt-pty/src/lib.rs",
        concat!("std::process::Command", "::new(\"ps\")"),
    );
    ...
    fn no_command_is_built_outside_the_quiet_door() {
        ...
        let door = root
            ...
            .join("src")
            .join("lib.rs");
        let (table_reader_file, table_reader_call) = PROCESS_TABLE_READER;
        let table_reader = root.join(table_reader_file.replace('/', std::path::MAIN_SEPARATOR_STR));
        ...
        for file in shipped_sources(&root) {
            ...
            let mut built = doors_built_in(&text);
            if file == table_reader && text.contains(table_reader_call) {
                built = built.saturating_sub(1);
            }
            if built == 0 {
                continue;
            }
        ...
        assert!(
            std::fs::read_to_string(&table_reader)
                .is_ok_and(|text| text.contains(table_reader_call)),
            "the one named exception is gone from {table_reader_file}, so the exception should              go with it"
        );
        assert!(
            outside.is_empty(),
