...
pub struct PlatformChrome {
    ...
    /// the owner's ruling of 2026-09-12, which is `docs/DESIGN.md` §13.20.
    pub band_px: i32,
    ...
    impl DirWatch {
        ...
        fn start_scoped(
            ...
            let thread = crate::spawn_at_priority(
                ...
                move |_ctx| watch_loop(dir, change, stop, depth, armed, wake),
            );
...
mod custom_frame_tests {
    ...
    /// **The boxes the application draws in the bar these tests describe**
    /// (§13.11 ⑥): a tab strip whose run ends at 240 logical pixels, and a
    /// caption run of four 46-wide buttons ending on the window's own edge.
    /// Everything else on the bar is the window's to be dragged by, which is
    /// what the cases below strike.
    fn app_boxes(dpi: u32) -> Vec<[i32; 4]> {
        let title = logical_px_for_dpi(40, dpi);
        let width = logical_px_for_dpi(960, dpi);
        let button = logical_px_for_dpi(46, dpi);
        let mut boxes = vec![[0, 0, logical_px_for_dpi(240, dpi), title]];
        boxes.extend((0..4).map(|index| {
            let left = width - (4 - index) * button;
            [left, 0, left + button, title]
        }));
        boxes
    }

    fn metrics(dpi: u32, app_boxes: &[[i32; 4]]) -> CustomFrameMetrics<'_> {
        CustomFrameMetrics {
    ...
    fn custom_frame_hit_test_maps_drag_resize_edges_and_caption_buttons() {
        let boxes = app_boxes(96);
        let m = metrics(96, &boxes);
    ...
    fn custom_frame_hit_test_is_dpi_scaled() {
        for dpi in [96, 120, 144, 168, 192, 240] {
            let boxes = app_boxes(dpi);
            let m = metrics(dpi, &boxes);
            assert_eq!(
    ...
    fn maximized_frame_has_no_resize_hits_but_keeps_caption_dragging() {
        let boxes = app_boxes(144);
        let mut m = metrics(144, &boxes);
        m.resizable = false;
        m.resize_border = 0;
        assert_eq!(custom_frame_hit_test(m, 0, 0), CustomFrameHit::Client);
        assert_eq!(
            custom_frame_hit_test(m, logical_px_for_dpi(240, 144) + 1, 1),
            CustomFrameHit::Caption
        );
        assert_eq!(
