# BetterTerminal shell integration for bash.
#
# Emits the same two things `betterterminal.ps1` emits, so that a bash pane is
...
#
#   * BetterTerminal starts a Git Bash or WSL profile with `--init-file <this>`,
...
#
# Nothing here is BetterTerminal-specific except the comments: `OSC 133` and
...
# at all unless the login flag is dropped. So BetterTerminal drops it and this
...
# sub-delims plus `:`, `@` and `/`, which is the same set `betterterminal.ps1`
...
__bt_refresh_cwd() {
    ...
    if [ -z "$place" ]; then
        ...
        # same rule `betterterminal.ps1` follows off a non-filesystem provider.
        __bt_cwd_uri=''
        return
    fi
    __bt_encode "$place"
...
# outranks the working directory in BetterTerminal's own name stack, so a pane
