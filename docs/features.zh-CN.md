...
- agent 打印的公式可能到达终端两次：一次是回答流式输出时，另一次是工具用自己的 markdown 渲染器重绘完成后的回答时。流式输出的字节正常排版。最终的重绘会改变它们——Claude Code 和 Codex 都一样：`\\` 变成 `\`、`\,` 变成 `,`、`\[` 和 `\]` 变成 `[` 和 `]`、`\!` 变成 `!`、仅含 `=` 的行把上一行变成标题、`$$` 行可能变成 `# $$`——于是刚刚排好的矩阵、带括号的展示式或带间距的积分退回到留存下来的文本。这是渲染器的行为，不是终端的；agent 写入 markdown 文件的公式完整到达，预览窗格正常排版。
...
- **导出设置**把设置、配置文件、快捷键和配色文件夹中的所有配色写入一个 JSON 文件。文件以固定顺序排版，两次导出可逐行对比。
- **导入设置**读取导出文件并立即生效。每部分按手动编辑时的规则读取：Folio 拒绝的快捷键或无法解析的配色标为已跳过，以卡片列出；其余正常应用。当前平台没有对应行的设置（Mac 上的**亚克力**、Windows 上的 **Option 键发送 Alt**）标为本机不适用，保留在文件中。
- **设置文件夹**打开这些文件所在的文件夹——Windows 上是 `%APPDATA%\Folio`，macOS 上是 `~/Library/Application Support/Folio`。

两台机器保持一致，可以用文件夹同步工具搬运导出文件或整个文件夹。

导出文件包含配置文件的原样内容，包括命令行和环境变量——分享前先检查。
