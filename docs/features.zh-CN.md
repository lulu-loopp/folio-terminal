...
- agent 打印的公式可能到达终端两次：一次是回答流式输出时，另一次是工具用自己的 markdown 渲染器重绘完成后的回答时。流式输出的字节正常排版。Claude Code 最终的重绘会改变它们：`\\` 变成 `\`、`\[` 和 `\]` 变成 `[` 和 `]`、`\!` 变成 `!`、仅含 `=` 的行把上一行变成标题——于是刚刚排好的矩阵、带括号的展示式或带间距的积分退回到留存下来的文本。这是渲染器的行为，不是终端的；agent 写入 markdown 文件的公式完整到达，预览窗格正常排版。
