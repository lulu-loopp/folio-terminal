# The icon round that was run, and not taken

The mark Folio ships is `assets/app-icon/folio.ico`, and the README beside it
says how it is drawn and how it reaches the binary. This directory is the round
that was run before it was kept: five hand-set candidates, the contact sheet the
choice was made from, and the two scripts that build an `.ico` from a drawing
and redraw the sheet.

**Retired on 2026-09-06**, when the ruling was to keep the drawing that was
already in the binary. Nothing here is a live option; it is kept because the
five arguments are the record of what the mark was weighed against, because a
mark that was chosen over something is easier to defend than one that was never
compared, and because the last section — what each one costs at 16 pixels — is
the part a future round should start from rather than rediscover.

| File | What it is |
| --- | --- |
| `candidates/{a..e}.svg` | Five directions for a replacement mark, hand-set in plain SVG, none of them chosen. |
| `candidates/{a..e}.ico` | Each of those five built out to all nine sizes. |
...
standing decision from the wordmark study (`docs/design/wordmark-r2/DECISION.md`):
...
```
python docs/design/app-icon/make-ico.py docs/design/app-icon/candidates/e.svg
```
...
```
python docs/design/app-icon/make-ico.py candidates/e.svg folio.ico
python docs/design/app-icon/make-ico.py candidates/e.svg --browser "C:\Program Files\Google\Chrome\Application\chrome.exe"
```
...
```
python docs/design/app-icon/make-candidates-board.py
```

If a future round ever does replace the mark, the winner's `.ico` is built with
the first command above and written to `assets/app-icon/folio.ico`, which is the
one path `crates/bt-app/build.rs` reads.
