...
### 调度活性洞:丢失的 completion 卡死 Pending(2026-07-25 修 + 诊断基建)

- **审因翻案**:上一单假设「版本拒绝把记录留 Pending,滚动不救、resize 救」。**静态证伪**:
  每次 VersionStamp bump 都与一次 per-record 复位配对(`source/detector/layout/view_changed` 全
  把 `Pending→None`,见 `four_versions_have_distinct_invalidation_boundaries`),`worker_task_is_current`
  又先门控——**版本门是自愈的,永不留 Pending**。真活性洞在上一层:**completion 被丢弃但版本没动**。
  最可达路径 = `session.rs` `apply_worker_completion` 的 `block_is_current == false`(2333):多行块
  扫描在飞行中,其某源行被删(scrollback 驱逐 / ED3 清史),投递的结果按输入快照失配被丢。scheduler
  无 in-flight/retry 账(`take` 后即无记录),`schedule_scan` 对非 `None` no-op → closer 永卡 `Pending`
  显示源码;唯有宽度 resize 的 `layout_changed(Pending→None)` 能救 = 用户「滚不救、resize 救」。
- **修法**(确定性,无定时器):`reopen_stranded_pending(candidate_id, versions)` — 任一 frozen
  completion 被丢弃(`complete_worker_task`/`complete_worker_result` 的 `!accepted` 分支)时,**若
  candidate 仍是我们调度的那一次尝试**(`Frozen` + `Pending` + versions 相等),复位 `Pending→None`,
  让既有可见页调度自然补发(从当前源码重建 inputs,收敛不 livelock)。绝不接受过期结果、绝不动
  已经改换状态(None/Ready/Failed/Suppressed/新版本 Pending)的记录。钉死测试
  `dropped_worker_completion_reopens_stranded_pending_instead_of_freezing_it`(修前红 `Some(Pending)`、
  修后绿 `Some(None)`)。
- **Failed 态裁决(不修,论证保持)**:`Failed` 同样滚动不救、`layout/source/detector_changed` 才
  复位。但 `Failed` 是**已记录的确定答案**(bt-math 渲染错误,同源同错,如 `\hbar`/MiTeX),不是
  丢失的答案;重试同源只会失败风暴。它已在任何真实失效(源码变更→`source_changed`、换检测器、
  换布局)上自动 `→None` 重试。语义线:**Pending=「还没答案」必须保持可重排;Failed=「已有(负)
  答案」正当终态直到输入变**。故无同构洞,保持。

### 真机诊断基建(下次卡死直接定名,绕开「回放不复现竞态」)

- **`BT_DECOR_TRACE=<path>`(运行时,bt-app)**:构造时读一次 env,未设 = `trace_decorations` 一次
  `Option` 检查即返回,**热路径零开销、不碰任何东西**。设了则 `publish_frame_inner` 每帧(含
  held/skipped 帧,持久卡死也能抓到)append 一个快照。格式:
  ```
  DECOR_TRACE frame=<n> epoch_us=<wall> screen=<Primary|Alternate> resize=<active|idle>
    FROZEN id=<n> state=<none|pending|ready|failed|suppressed> reason=<...|-> src="<≤96 char 摘要>"
    LIVE row=<r> band=<a>-<b> state=<rendered|stale|failed|pending> reason=<...|-> src="..."
  ```
  只打「有意思」的 frozen 记录(decoration≠None 或源行仍是 display-math 候选),外加全部 live 记录。
  用户复现卡死时,卡死块的确切 state 直接在文件里(`pending`=本次修的活性洞、`failed`+reason=
  bt-math 错、`none` 候选=没重排上)。无锁、只读两张 map。
- **`BT_PROBE_FROZEN` 扩展(回放侧,oracle)**:每 `FROZEN[id]` 行加 `state=<...> reason=<...>`,
  与运行时同一套 `decoration_state_label` 标签(`bt_term::decoration_state_label` 导出)。
- **保留上一单未提交的 `BT_PROBE_REVIEW`**(oracle 结算后全文档扫读装饰态、只读不新调度,卡死块以
  源码行浮现),一并交付。
