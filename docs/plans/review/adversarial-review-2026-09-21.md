...
| D2-2b | high | Not a knife of this audit: found on 2026-09-25 by the ownership census (§4.1 V2) and traced by its Codex review (R1), recorded here beside its failure-road cousin. An OS close requested while the unsaved-changes gate is up — over a tab's close still being asked about, or a second Alt+F4 — was read as "nothing to ask", because the gate answered a busy request with the same `false`; the window closed and the edited buffers went with it. A tab whose shell exited beside a dirty preview lost them the same way a turn after its question appeared. | **true** (static trace; the decision reproduced on real tabs over a real file in the tests). | **fixed in 0.4.5 by ticket 58**, entry *A busy dirty gate is never read as "nothing to ask"*: the gate answers raised / nothing to ask / busy, and only nothing to ask lets a verb go on. D2-2 itself stays on the ledger (D-4, 0.4.6). | `a_window_close_requested_while_the_gate_is_up_neither_closes_the_window_nor_loses_the_buffer`, `two_os_close_requests_in_a_row_ask_once_and_close_nothing_until_answered`, `cancel_keeps_the_buffer_and_save_and_discard_replay_the_accepted_request`, `nothing_to_ask_still_closes_at_once`, `every_producer_of_the_dirty_gate_goes_on_only_when_there_was_nothing_to_ask` |
| D2-2c | high | Not a knife of this audit: found on 2026-09-25 by ticket 58's caller audit. When the last ordinary window closes, `FolioApp::close` retires the quick terminal with the run (`retire_the_summon_with_the_run` calls `Runtime::close_window(true)` directly), so an unsaved preview edited in the quick terminal is dropped without the unsaved-changes question ever being put. | **true** (static). | **open — 0.4.6, beside D-4.** | — |
