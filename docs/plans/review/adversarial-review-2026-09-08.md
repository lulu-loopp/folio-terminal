...
All nine tickets were merged to main on 2026-09-08, the last at `5516799`, and shipped in v0.2.4-preview (tag `2d05ae3`, 2026-09-09). The T8 test-isolation follow-up (`c6a53a2`, a store asks whether it is the writer of its own directory) shipped in v0.2.5-preview (tag `251a603`, 2026-09-09). Each heading below records its merge commit and what it left open.
...
### T8 `fix/persisted-state` (merged, `663a46c`; second instance = session-local named mutex on the data directory, the second process keeps its window and writes nothing, and the product decision taken on 2026-09-08 is that Folio becomes single-instance after 0.2.4, the second launch handing its command line to the running process — **that behaviour now exists**, shipped on `feature/single-instance` and written down in `docs/DESIGN.md` §7.59: the channel is a second well-known named pipe beside the attention endpoint (`crates/bt-platform/src/launch_pipe.rs`, grammar in `crates/bt-app/src/launch_wire.rs`), its name folds in the same data-directory digest this row's mutex does, and the mutex is still what decides — a second *process* starts only where this claim is free, or where the running one could not be reached inside two seconds; left open: R4-6's spoken half, `storage_dir()` as a process-wide OnceLock keeps several readers out of tests, the rejected-bytes copy is taken at startup only)
