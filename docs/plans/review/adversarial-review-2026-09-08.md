...
### T8 `fix/persisted-state` (merged, `663a46c`; second instance = session-local named mutex on the data directory, the second process keeps its window and writes nothing, and the product decision taken on 2026-09-08 is that Folio becomes single-instance after 0.2.4, the second launch handing its command line to the running process; left open: R4-6's spoken half, `storage_dir()` as a process-wide OnceLock keeps several readers out of tests, the rejected-bytes copy is taken at startup only)
