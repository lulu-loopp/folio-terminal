...
- **一个对象只有一个名字。** 同一件东西在行名、说明、菜单、快捷键表里必须是同一个词。中文尤其：`profile` 全表写「档案」，不许一处「档案」一处「配置文件」；`integration` 全表写「集成」，不许一处「集成」一处「整合」。
...
**不在禁语表里的**（它们是读者的词，不是内部词）：`Search engine`／搜索引擎、`WebView2 Runtime`（微软的产品名）、`hook`／`hooks`（上游产品自己的叫法）、`profile`／档案、`pane`／窗格、`tab`／标签。
...
- 红门：`settings.rs` 的 `no_settings_sentence_needs_a_fourth_line`，按设计的 118px 控件列量，报出**每一条**超长的行名。
