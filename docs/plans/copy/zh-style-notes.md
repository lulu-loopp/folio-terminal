...
**目前没有豁免的描述**（用户裁决，2026-09-23）。`Option key sends Alt` 的英文改成了两行：`On, Option is Alt to terminal programs. Off, it types accents as in other apps.`，豁免随之取消，`OWNER_RULED_EXCEPTIONS` 现在是空的。**中文同样受两行预算约束**，这一句的中文待写（`Text::CHINESE_PENDING`），写时保留英文里的两个事实。豁免机制仍在：每条裁决一个条目，没有其他办法获得豁免。下面一段是被取代的旧裁决，留作记录。
