...
**No sentence is exempt today** (owner ruling, 23 September 2026). `Option key sends Alt` now reads `On, Option is Alt to terminal programs. Off, it types accents as in other apps.` — two lines at 235px, with both facts kept — and it came off `OWNER_RULED_EXCEPTIONS`, which is empty. The layout's tolerance `ROW_DESC_MAX_LINES` went from six to four: one line past the floor pins' three, so they can still count a fourth. Exemptions are still entries on that list, one per ruling; there is no other way to be excused, an entry that stops needing the exemption fails the budget test by name, and none is added without a ruling. The paragraph below is the superseded ruling, kept for its reasoning.

~~**One sentence is exempt, by ruling and by name.** `Option sends Alt` takes as many lines as its own column needs and keeps every one of them — four in Chinese and six in English at the narrowest column, 235px, which is the one the General page draws (owner rulings, 13 and 14 September 2026: the sentence is byte-exact, so the layout's tolerance `ROW_DESC_MAX_LINES` is set from the longest ruled exception, measured, rather than the sentence being cut to a tolerance): the two-line rewrite bought its length by dropping which programs want Option to be Alt, and that the characters it types otherwise are the ones every other Mac app types — and the reader this row exists for is one whose `⌥a` has just done the wrong thing. Exemptions are entries on `OWNER_RULED_EXCEPTIONS` in `settings.rs`, one per ruling; there is no other way to be excused, and an entry that stops needing the exemption fails the budget test by name. Do not add one without a ruling.~~
