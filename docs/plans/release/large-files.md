...
| 批 | 张数 | 总字节 | 结论 | 理由 |
|---|---:|---:|:--:|---|
| `docs/screenshots/*.png` | 21 | 3,648,539 (3.48 MiB) | **keep** | README 两语共用的实拍图。2026-08-27 由 100%(1600×1000)改拍 200%(3200×2000),字节数约翻四倍——**这是明知的代价**:README 主要在 1.25×–2× 的屏幕上读,1x 图在那里是放大过的图。每张都过 `oxipng -o 4 --strip safe`(无损,省下 19.7%:3,192,930 → 2,564,435)。0.2.0 新增四张(快捷终端、聚焦搜索各双主题),其中 `quake-*` 取 1920×1200——同为 200%,只是取景框小,理由写在 `docs/screenshots/README.md` 里。0.2.1 新增一张 `context-menu.png`(786×593,55,470 字节),它是唯一一张不在本机拍的:一级菜单那一行只在登记了 `folio.msix` 的机器上存在,而本仓库任何东西都不在打包机上登记包,所以它出自验证用的干净虚拟机,按那台机器自己的 100% 取景,不放大。0.2.2 新增两张 `first-run-*.png`(3200×2000,合计 178,565 字节):初次设置卡每台机器只出现一次,所以它们是唯一一对靠 `BT_FIRST_RUN_CARD` 那扇门拍的,机器状态全部隔离,取景与其余 200% 那批一致。规格与逐张哈希见 `docs/screenshots/README.md` |
...
两批合计 **4,834,266 字节(4.61 MiB)**,占一次 clone 的约 4.3%。
