...
| 批 | 张数 | 总字节 | 结论 | 理由 |
|---|---:|---:|:--:|---|
| `docs/screenshots/*.png` | 18 | 3,414,504 (3.26 MiB) | **keep** | README 两语共用的实拍图。2026-08-27 由 100%(1600×1000)改拍 200%(3200×2000),字节数约翻四倍——**这是明知的代价**:README 主要在 1.25×–2× 的屏幕上读,1x 图在那里是放大过的图。每张都过 `oxipng -o 4 --strip safe`(无损,省下 19.7%:3,192,930 → 2,564,435)。0.2.0 新增四张(快捷终端、聚焦搜索各双主题),其中 `quake-*` 取 1920×1200——同为 200%,只是取景框小,理由写在 `docs/screenshots/README.md` 里。规格与逐张哈希见 `docs/screenshots/README.md` |
