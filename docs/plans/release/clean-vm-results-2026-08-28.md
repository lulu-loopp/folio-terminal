...
1. **§5.1、§5.2、§5.3 三条发布阻塞项均已修并各自在干净机上复跑通过**(2026-08-28:§5.1 见 `docs/DESIGN.md` §7.35,Win11;§5.2 与 §5.3 见 §7.36,Win10)。**§5.4 已撤销**,不是缺陷。门 5 需在合并后的同一个二进制上把两台再各跑一遍,才算过。
...
## 复跑判定(2026-08-28,二进制 `0.1.0 (36439653f8)`,门 5 三红修复后)

**门 5 通过。** 两台干净机各拿修复后的发布 zip(next15 二进制)复跑冒烟,均 exit 0:
- Win10 Pro 19045(无网卡、无 WebView2):缺席卡出现(§5.2 修复,§7.36)、开座窗 chrome 文字回到框内含 PSReadLine 对话框(§5.3 修复,§7.36)、干净关闭 web.txt `exit code : 0`。
- Win11 Enterprise Eval 26200(WebView2 151):真网页渲染、地址栏正确、干净关闭 web.txt `exit code : 0`(§5.1 修复,§7.35;此前为 −1)。
- 两台 `--version` = `Folio 0.1.0 (36439653f8)` 与 PE 资源一致;ConPTY 用 sidecar;DPI 对齐(96)。

**未阻塞发布的挂账(记录不修或另排)**:缺席卡细节行比 479px 座宽长、两端被裁(卡片排版旧账);`javascript:` 无拒绝卡;通知条 700px 相撞;SmartScreen 需真浏览器下载的 MotW,本 harness 拍不到(发布前用真下载在 Win11 上人工过一次);客户机 1024×768(窗口摆位已避裁,非分辨率问题);截图不入库(含账户名 `folio`)。
