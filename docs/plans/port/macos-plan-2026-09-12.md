...
> **Result 2026-09-12 (M1-1, `feature/macos-startup-path`): the startup path runs on macOS and a window presents through Metal. The shell half of this acceptance is not reachable from M1-1 and the reason is a sequencing fact rather than a defect — see the last paragraph.**
> `cargo build -p bt-app` links on the Mac. Launched from the bundle this ticket assembles (`packaging/macos/Info.plist.in` rendered into `Folio.app`, `open` because an ssh session cannot reach the window server), the process reaches a frame: `BT_STARTUP alpha target=Hwnd offered=[Opaque, PostMultiplied] chosen=Opaque`, `BT_DPI stage=create/show/resized … swapchain_size=960x600`, `background_visible=371ms`. Nothing on the path is fatal any more: the four items that have no macOS arm yet report one line each and the launch continues — the backing colour, the outer rectangle, the dark-mode statement and the two directory watches. `NativeWindow` is the door every one of them goes through; `window_surface_target` is the one function M1-4 replaces, and today it hands wgpu the window itself.
> **Two measurements for the tickets after this one.** `winit_scale=1` and `authoritative_scale=1`: the window opened on the second display the owner attached for §8 Q11, which really is a backing-scale-1.0 panel beside the 4K one — so this is the mixed-scale desktop that ruling asked for, and M1-3 and M2-5 now have both scales to measure on. And closing the last tab reports `request close after the final tab: asking a window to close is not on this platform yet` and ends the run at exit 0: `request_window_close` is M1-3's, and its refusal is on a `?` outside the startup path that M1-3 should take with it.
> **The shell: the first pane's program does not come from M1-6's rule, and nothing in `bt-app` routes it there.** It comes from `profiles::shipped()`, which is a list of Windows profiles — `pwsh`, `winps`, `wsl`, `cmd` — so on macOS the default profile resolves to nothing and the pane stays empty. Handed `BT_SHELL=/bin/zsh`, the same build spawns a real child on a real macOS pty and records what it wrote (`BT_PTY_DUMP`, 32 bytes, two chunks), which is `/bin/zsh: no such option: Logo` — the `pwsh` profile's `-NoLogo` reaching zsh. So M1-6's children work inside the application, and the whole of what stands between this build and a `zsh` prompt is **M1-5**, the native shipped profiles. The graph does not say so: M1-5 depends on M1-6 and M1-1 does not depend on M1-5, but *this acceptance line* does, and M1-5 is the ticket that ends it.

> **Result 2026-09-12 (M1-5, `feature/macos-shipped-profiles`): the first pane on a Mac is a real zsh, standing in the home directory, with its shell integration in force. The shell half of this acceptance line is met; the clipboard, the interrupt, the IME and `Cmd+T` are M1-7's, M1-8's and M1-9's.**
> `profiles::shipped()` is the platform's table now, and all three tables are compiled into every build (`shipped_for(platform, environment)`) so that what a macOS window ships is a claim a Windows runner can check — which is exactly the claim that went unchecked until M1-1 found the pane empty. macOS ships `zsh` (`/bin/zsh`), `bash` (`/bin/bash`) and `sh` (`/bin/sh`); another Unix ships the last two, because a Linux userland keeps zsh under `/usr/bin` as often as `/bin`. Each is interactive and non-login with **no arguments at all**, which is M1-6's `UNIX_INTERACTIVE_ARGS` read one layer up. `$SHELL` reaches the table under one rule — the row that starts a shell of a given name starts the one this account actually runs, so a Homebrew zsh is still the `zsh` row and a `fish` gets a row of its own — and a case pins that the table and `resolve_default_shell` answer program for program. The floor off Windows is `/bin/sh`, and no arm off Windows names a PowerShell (pinned over the rows, over the floor, and over the source of the table itself).
> **The proof, and which half came from where.** Accessibility is not granted, so no key was posted at the window: both halves are read out of the pty recording. ① **The ordinary launch** — the bundle, a fresh data directory (`APPDATA` under the worktree; `storage_dir()` is still `%APPDATA%`-shaped, which is M2-6's), `BT_PTY_DUMP` — recorded 219 bytes in four chunks: `OSC 7 file:///Users/<owner>`, `OSC 133;A`, the prompt `<user>@<host> ~ %`, `OSC 133;B`, `CSI ?2004h`. That is an interactive zsh with `folio.zsh` in force through `ZDOTDIR` (the three startup files were written into the data directory), reporting **the home directory** as its working directory; `ps` shows the child as `/bin/zsh` on `ttys001` with no arguments. ② **The acceptance line's own two questions**, answered by the same build with a throwaway `profiles.json` overriding the `zsh` row's arguments with `-c`: `echo $0` → `/bin/zsh`, `pwd` → `/Users/<owner>`, `ls` → the home directory's listing. `$0` is the program's path rather than the bare word because argv\[0\] is the path and this is not a login shell; `-zsh` is what the login form answers, and Q8 ruled that out.
> **The environment a pane gets.** `TERM=xterm-256color`, `COLORTERM=truecolor`, `TERM_PROGRAM=Folio`, `TERM_PROGRAM_VERSION=0.3.0` — read back out of the pane, and the same four on every platform from one `cfg`-free function, now pinned. **`LANG` is unset on this machine and that is the rule working**: `defaults read -g AppleLocale` answers `zh-Hans_US`, and neither `zh-Hans_US.UTF-8` nor `zh_US.UTF-8` is among this machine's 288 installed locales, so Folio declares nothing rather than choosing `en_US.UTF-8` — a country and a language — on the owner's behalf. `PATH` arrives exactly as the launch handed it over and Folio does not repair it: a non-login zsh reads `.zshrc`, which is where a macOS reader's `PATH` comes from, one layer inside the pane.
> **Two things this run also says.** The launch that ran to the end reports `request close after the final tab: asking a window to close is not on this platform yet` and exits 0 — M1-3's, as M1-1 already recorded. And **`cargo test -p bt-app` still does not build on the Mac**: two errors, both in `cli.rs`'s test half (`use std::os::windows::ffi::OsStringExt` and `OsString::from_wide`, at `cli.rs:1262`), which is the gap M1-2's inventory named at `cli.rs:1240` — "the production function is portable, its test has no portable twin" — and which M1-10 owns. So this ticket's own cases are written to be run on either host: the three seed tables and the home-directory variable are parameters rather than `cfg` bodies, and all five named cases run on the Windows gate. `cargo test -p bt-platform` runs on the Mac and the locale cases pass there.
