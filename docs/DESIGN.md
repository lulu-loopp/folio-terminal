...
**④ 用户日志里为什么写的是 overlay,而丢字的是 preview。** 不是两件事,是**一件事只被记了一次**:那次运行用的还是 `glyph_degraded_frames` 那个**一个计数器管五条泳道**的版本(§7.1.3l ③ 已改成按泳道各记一行),所以**整个进程只有第一次拒绝会印出来**。那一次恰好发生在 overlay 泳道——某个更早的时刻某张悬浮卡——之后每一帧的 preview 拒绝一行都没有。日志说的是「这个进程第一次变哑的是谁」,不是「这一帧丢字的是谁」。preview 的字**没有**「被逐出后没重铸」的第二种毛病:一条泳道的字每帧都重新 prepare,拒绝是整批失败而不是漏掉几个。
