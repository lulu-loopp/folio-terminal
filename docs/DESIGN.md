...
**边界表(§7.1.5j)补十二行(40–45 为 2026-08-27,46–49 为 2026-08-28 下午,50–52 为 2026-08-28 晚的开括号一条,并就地推翻第 48 行)。** 与前面 39 行同一张表的续号;每一行一条测试。
...
| 52 | `src/app.ts(7,19): error TS2322`(tsc/MSVC 括号位置)、`src/{old => new}/main.rs`(git rename 压缩) | `src/app.ts` / `src/`(**均为新认**) | **开括号这条规矩的连带**,原「不认风险」场景 7、8、44 判「整项不画线」。两条的新答案都不是场景禁止的那种前缀:画的是 `src/app.ts`(路径本身,正是场景 7、8 写的「只画路径」,只差一个还没有契约的 `(12,34)` 片段)与 `src/`(真在盘上的目录),而 `…main.cpp(12,34`、`src/{old`、`new` 一个都没被产出过。场景 44 的「无链接」是这条规矩在这里的代价,裁决知情地付了:更短的读法永远靠磁盘持有它才被画,不靠对语法的猜测 |

**红门与红证(先写、跑红、贴原文,再改)。** 十条在 `bt_transcript::paths`(第十条为 2026-08-28 晚的开括号),另有一条断言加在 `bt_term::inline_image` 的既有测试里(⑦):
...
**这条裁决改判了三条既有测试,每一条都是它自己的判词被推翻而不是被绕开。** `a_bracket_glued_to_a_cjk_word_seams_like_any_other_separator` 里第 48 行那句断言(「an ASCII `(1)` must not seam a name into a shorter one」)正是本条推翻的那一句;`group_a_quoting_provider_prefixes_and_position_syntax` 的第 7、8 行与 `group_d_git_and_virtual_schemes` 的第 44 行按边界表第 52 行改写。三条都留着改判的理由写在测试里,好让下一个读到它们的人知道旧判词去了哪儿。
