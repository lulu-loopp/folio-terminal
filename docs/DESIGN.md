...
**7.1.3k markdown 预览里的图片(用户实证 2026-08-28「md 预览不渲染图片」:`README.md` 的 `<picture>` 整段当文字印出来,`![alt](path)` 印成一个 `!` 加一条链接;`crates/bt-app/src/{preview,preview_select,main,i18n}.rs`)。** 报告属实,而且这是一个**渲染 markdown 的产品**身上的硬缺口。**病与 7.1.3i′ 的公式同源、也同解**:解码道早就有(§7.1.3j 的图片面、hover 一瞥卡、终端的 inline image 走的都是 `bt_term::InlineImageDecoder` + `bt_term::scale_inline_image` 这一对),病在 **md 道从来没有接过它**。所以这一片一个新依赖都没有引,一条新的解码路径都没有开。

**① `![alt](src "title")` 是一张图,而且它是在**链接那一趟**里被认出来的。** CommonMark 里图片不是第二种语法,它是链接的语法**前面加一个 `!`**;所以判据落在 `push_link_runs` 里,而不是一趟自己的扫描——两趟扫描就要各自同意「标签在哪结束」「什么算转义」,而它们迟早会不同意。`!` 前面有奇数个反斜杠就是作者自己的叹号(与美元、与星号**共用同一个** `bt_detect::delimiter_is_escaped`:一个定义,四处调用)。`[]()` 仍是标点,`![](x.png)` **是**一张图——空 alt 是文档在说「这张图没有话要讲」(间隔、横线、一枚自己就是意思的徽章),那是一张图而不是一个空标签。顺带修掉一条同源旧账:`(dest "title")` 里的标题过去被整段当成地址交给磁盘,现在 `link_destination` 一个函数**同时**给图片和链接剥掉标题、解开 `<…>`——同一套语法,一个读者。标题本身**解析掉、不显示**:在浏览器里它是 tooltip,而这扇窗不给图片挂 tooltip,留着就是留一个没人读的字符串。

**② 内联 HTML 只认 `<img>` 与 `<picture>`,其余原样印。** CommonMark §4.6 的 HTML 块 6 型:行首是这两个标签之一就开块,到空行为止。**整段必须**就是**这张图**——一段里既有图又有一句话,那是这扇窗画不出来的 HTML 块,原样印出来是对它诚实的答案(`html_tags` 见到标签之间有非空白就整段拒绝)。`<picture>` 按**当前主题**挑 `<source>`:先找 `media` 指名本主题的,再找没写 `media` 的,都没有就落到 `<img src>`——这正是浏览器「没有一条匹配就用 `<img>`」的规矩。**一条这扇窗答不出的 media(`(min-width: 600px)`)不是「无主题」而是「不可选」**,直接落选:把答不出的条件当成通配,是渲染器替文档回答它没被问到的问题。`srcset` 取列表第一个地址(密度描述符问的是设备像素比,而一张画到栏宽再按落地像素重采样的图已经用另一种方式回答过它)。`width="100%"` 是唯一被读的宽度属性,它**只**决定要不要填满(见③);别的写法读掉不用——一个 markdown 预览不是百分比排版引擎。alt 里的换行按 HTML 的规矩折成空格(本仓库 hero 的 alt 就是五行一句话)。

**③ 一张图画到栏宽、保比例、**永不超过自己的像素**。** `markdown_image_extent(native, measure_px, fill)` 是**唯一**说这句话的地方——排版留的框、画家填的框、重采样问的尺寸全走它,所以三者不可能互相不同意(§7.1.3j (a) 对图片面说过的同一条纪律,说给页面听)。3200 像素的截图落到 400 的栏里是八分之一,96 像素的徽章就站在 96 像素、居中;`fill` 只在 `width="100%"` 时抬起后半句。**图片块永远不会比栏宽,所以它没有自己的横轴**——这与表格、围栏、display 公式三者不同,是上面那条钳位的**结果**而不是另一条规矩。

**④ 远程图片不联网取,这是政策不是缺口。** `README.md` 与 `PRIVACY.md` 写着这个程序**没有自己的网络客户端**,所以一个 `http(s)://` 的 `src` 不是「拒绝去取」,而是**没有办法去取**。落地上这一条是**结构性**的而不是行为性的:`resolve_document_pictures` 是一个**把磁盘注入进来**的自由函数,`ask` 是它通往文件的**唯一**一扇门,于是红门 `a_remote_image_is_never_fetched` 自己拿一扇门、数有几次到达,断言的是「一个远程 src 不产生任何读取、任何请求、任何道上的流量」——而不是「今天的代码碰巧走了别的分支」。读者拿到的是 alt 文字加那条地址,**地址是一条链接**,走这扇窗的老规矩(点=留窗内开、Ctrl+点=交出去,见 §7.20)。

**⑤ 解析出来的 `src` 由 `preview::link_action` 解析成路径——那是链接用的同一个解析器。** 相对路径相对 **md 文件自己所在的目录**,`file:` 认,绝对路径认,别的 scheme 一律不开。一张图的 `src` 和一条链接的 target 是同一份文件里同一种字符串,给第二种写第二条规矩,就是给「`../assets/a.png` 相对于什么」准备两个答案。

**⑥ 两条道回答一张图,次序是固定的。** 解码道(`request_peek_pixels` → `peek_cache`,与图片面、一瞥卡**同一个**缓存、同一把路径钥匙)说这个文件**自己的像素**有多大——块的形状从这里来;重采样道(`ScaleWorkerRequest::MarkdownImage` → `bt_term::scale_inline_image`)再说这些像素**画到那么大**是什么样。第二个答案没到之前,交出去的是解码自己的栅格、由采样器拉伸——**正在变清晰的图永远不会消失**,这是 §7.1.3j 拖窗时说过的同一句话。**精确尺寸那一趟等静默**(§7.1.3j (d) 用在页面上):拖窗把一份文档走过上百个宽度,每个宽度一趟 Lanczos3 就是九十九趟问的是手已经离开的尺寸。欠账**按 content key 记**(`MarkdownPictures::owed`),所以拖过的每个宽度覆盖同一条,最后发出去的是手停下时那一条。

**⑦ 缓存是窗口的、按内容与尺寸记、按字节封顶。** `MarkdownRasterKey{content, width, height}` 里的 `content` 是**文件自己字节的哈希**(解码器铸的),所以磁盘上被重写过的文件是另一把钥匙,不存在「端上旧像素」这件事要另立规矩。预算 64 MiB、按最近使用逐出,逐出与到达同样 +1 代数(理由与 `PreviewMathCache` 一字不差:块的高度往两个方向变都要重排);**最后一张不逐**——逐了就是下一帧再问、再发现太大、再逐,一张永远画不出来又一直在花钱的图。

**⑧ 图片是预览的一部分,所以它跟着文件监视走。** `watched_preview_files` 现在把每一页 markdown 引用的图片文件也交给 `preview_watch`(此前那里写着「图片故意不监视」,那句话是对**图片面**说的——那扇面看的就是那个文件;而文档里的一张图是另一种东西:读者打开的是文档,图是它怎么读的一部分)。一个被写过的图片文件到达时丢掉 `peek_cache` 里那把路径钥匙下的条目并 +1 代数,每一页站在这个文件上的文档因此重新问一遍。

**⑨ 一张图复制出来是 `![alt](src)`,选区整块跨过它。** 与 display 公式复制 `$$…$$` 是同一条裁决同一个理由(§7.31 的「复制的是读到的字」在一张图上的读法):页面上是一张图,而一张图诚实的纯文本就是**被标记成图**的那个名字。只给 alt 会把作者从没写过的一句话贴成散文,而且贴出来的东西一点看不出曾经是图。`src` 用文档自己的字符串、不解析,所以贴回文件里文件还是原样。`atomic`,因为一张图里没有哪个偏移量对任何一边有意义。

**⑩ 画不出来时画 alt,并且说一句为什么;还在路上时只画 alt。** 「解码失败/文件不存在」画的是围栏那身衣服里的一张卡:alt 文字(没有 alt 就是 `src`,一张什么都不写的卡还不如一个文件名),底下一行 `MarkdownImageUnreadable`。它**不是** `PreviewFailedImageLoad`——那句话是图片**面**的,开头是「预览失败」;这里没有什么失败了,图周围的整页都画着、读得下去,只是其中一块暂时站在它说的话上而不是它给看的东西上。**还在解码时不写那一行**:「正在载入」不是关于这份文档的事实,而一张先解释自己、再被截图顶掉的卡就是一次闪烁。

**⑪ 一段散文在它的图片处被切开** —— 散文、图、散文。所以这扇窗的 markdown 里**每一张图都是一个块**,`![a](b)` 独占一段是最常见的情形(本仓库 `README.md` 的每一张都是),那时两边的散文是空的、块就是一张图。**挂账三条,写下来而不是留给下一个人发现**:(a) **真正的行内图**——按周围文字定大小、坐在句子基线上,机制 `bt_render::PreviewRun::inline_box_px` 已经为行内公式建好了,标题/表格单元格/列表项/引用行这四个切不开的地方今天画的是 alt 文字;(b) **`[![alt](src)](href)`**(徽章那种图外面套链接)今天仍按旧样子印;(c) **一张 SVG 被 `width="100%"` 要求填满一栏、而栏比它的固有尺寸宽时,是采样器在放大一张按固有尺寸光栅化的图**——按落地尺寸重新光栅化要让解码道认识「目标宽度」,那是解码道的事。

**⑫ 两件在实机上才露头的账,都不是这一片新造的,都是这一片第一次去碰。** ① **一份带字的 SVG 从前一个字也画不出来**:`bt_math::rasterize_svg_document` 用的是 `usvg::Options::default()`,那是一张**空的字体库**,usvg 对每一个排不出的 `<text>` 的处理是**默默丢掉**。这在从前看不见——走到这个函数的 SVG 只有 Typst 吐出来的那些,一份排好的公式是路径不是文字;它在一份 markdown 开始画作者自己的 SVG 那天才露头:本仓库 `README.md` 的 hero 是十六个 `<text>`,回来的是一张只有底色的图。修法是在那把 `OnceLock` 里 `load_system_fonts()`——一次,在已经为了把几十毫秒的排版挪出窗口线程而存在的那条道上付。红门 `an_svg_that_says_something_draws_it`。② **换主题的时候没有人叫页面重排**:`adopt_new_palette` 清了 rail 的缓存(那一行正是为同一个病写的)、刷了 chrome、发了一帧,唯独没有叫 `refresh_preview_for_layout`。页面上的**字**免费换色,而两样东西不会——公式的栅格是烤过色的,`<picture>` 是**一个主题一个文件**——两样都在 `PageArtKey` 里,正是为了让换主题成为一次不同的排版问题,而没有人去问那个问题。实机形状:chrome 变亮了,页面拿着暗色的 hero 和暗色的截图**一直不变**。红门 `a_theme_flip_asks_every_markdown_page_to_lay_out_again`(源码门:主体是要窗口、要 GPU、要调色板的 `Runtime` 方法,承重的是那一次**调用**在不在每一次换主题都要走的那一个函数里)。

**红证与门**:`preview.rs` 五条解析门(`a_picture_element_picks_the_source_for_the_theme_in_force` / `a_source_whose_media_is_not_about_the_scheme_is_not_chosen` / `a_bare_img_tag_is_a_picture` / `html_that_is_not_one_of_the_two_tags_is_printed_as_it_stands` / `a_bang_in_front_of_a_link_makes_it_a_picture` / `a_paragraph_is_cut_at_the_pictures_it_carries`)、`main.rs` 四条(`a_markdown_image_is_drawn_from_the_documents_own_directory` 一条门里管住「相对 md 目录解析」「块高=画出来的高」「小图不放大且居中」三件、`a_remote_image_is_never_fetched` 的结构门、`an_image_that_will_not_decode_says_so_in_place`、`a_picture_copies_the_markdown_that_named_it`)。实机语料就是本仓库的 `README.md`:hero 是一对亮暗 SVG,截图是一对亮暗 PNG,`<picture>` 在两个主题下各取各的那一张。

**7.1.3l 一页中文 markdown 只画出图、一个字都没有(用户实机报告 2026-08-29;`crates/bt-render/src/lib.rs`、`crates/bt-app/src/main.rs`)。** 现象:预览 pane 打开仓库根的 `README.zh-CN.md`,两张 `<picture>` 都画出来了,`---` 的横线、二级标题的下划线、引文左边那条竖条也都在,**中间是大片空白**——正文一个字母都没有,连 `[English](README.md)` 这一行拉丁字母和蓝色链接都没有。

**先说不是什么。** 同一个文件在这套装置里连开三次——系统浅色、系统深色、用户自己的 Solarized Dark 配色 + 中文界面——**三次都排得整整齐齐**,`BT_PREVIEW_TRACE` 三次都写 `built paragraphs=4 quads=3` / `frame … drawn=4 prepared=1 refused=none`,而且截图量出来的几何与用户那张**逐像素同形**(引文竖条 90px = 两行 × 45px,scale 2)。所以:CJK 字形进得了预览的字族栈(§7.1.3 的 `CJK_FALLBACK_FAMILIES` 那条链是好的),`<picture>` 没有吞掉后面的正文(`html_image_block` 到空行为止),配色没有把墨画成底,换行也没有把行推到看不见的地方。**版面是对的,墨没上去。**

**真因:共享字形图集(glyph atlas)满了,而一次拒绝带走的是一整条泳道的全部字。** 用户机器自己的 `%APPDATA%\Folio\diagnostics.log` 在出这张截图的那一次运行里写着 `Folio glyph atlas reached the device limit`。一帧里的字形按泳道分批交给 glyphon:**grid → chrome → preview → overlay**;一条泳道 prepare 过的字形在这一帧里是**不可逐出**的,所以谁最后问,谁就撞上一个满的图集,而 `PrepareError::AtlasFull` 不是「少画一个字」——它让 `TextRenderer::prepare` 整批失败,那条泳道这一帧**一个字都不画**。于是画面正好是报告里的样子:格子和 chrome 的字都在(它们先问),页面的填充与图片也在(它们不走字形图集),**只有页面的字没了**;而它上面那条四个字的 pane 头,因为排在 chrome 泳道,反而留住了自己的字。

**为什么是中文触发。** 图集按 (字面, 字号, 亚像素分格) 存一份位图,拉丁文一屏两百来个格子就够,汉字一屏是上千个;界面是中文、终端里是中文、文档也是中文的时候,同一批汉字还要按 chrome/正文/标题/迷你卡好几个字号各存一份。本机在 `max_texture_dimension_2d = 8192` 上量过这条阶梯:一帧要到约 2.5 万个不同字形位图还过得去,约 3.8 万就被拒——一个 4K、200% 缩放、全中文的窗口是**够得着**这个天花板的,英文的同一个窗口差一个数量级。

**改法两条,都不是「多要一点显存」——显存已经到设备上限了,能改的只有次序和收手。**

**① 页面的字比它周围的标签先要图集。** prepare 的次序不是 draw 的次序(`TextRenderer` 拿到批次后按 z 序另行 issue),所以这个次序**唯一决定的事**就是「不够分的时候谁让」。原来的次序是 chrome 在前、页面在后,报告里付账的就是这条:一个丢了字母的标签是一个不见了的标签,**一份丢了字母的文档不是文档**。现在页面先问,顶不住的时候让路的是它周围的家具。改动只是把 `compose_frame` 里 preview 那一段 prepare 挪到 chrome 之前,画的次序一行没动。

**② 「重试」得真的是重试,不能是原地空转。** `PresentedWithoutText` 的答复一直是「帧还欠着,放回槽里再要一轮」,理由是「图集每帧出门都 trim,下一帧就有地方了」。这句话只对**上几帧留下的占用**成立;当**这一帧自己**要的字形就超过设备上限时,下一轮是同样的座位、同样的 chrome、同样的文档,在同一个地方被同样地拒绝——窗口于是以最快速度重画一张一模一样的、同样缺字的画,永远。这不是重试,是空转:它烧一个核和一条 GPU 队列,也正是那次运行里挂起看门狗跟着响了 6.36 秒的由来。所以重试**记账封顶**(`TEXTLESS_RETRY_BUDGET = 2`:一轮覆盖 trim 能治的那种拒绝,两轮覆盖内容在同一口气里换掉的情形,第三轮就是把同一个问题问第三遍还当它是新的),满了就让窗口静下来;任何一帧完整落地就把这笔账清零。交换链自己的两种拒绝(`Skipped` / `Reconfigure`)不受这条约束——那两种情况下一轮会变的是交换链,不是这扇窗。

**③ 顺带:一次拒绝要按泳道各说一次。** `glyph_degraded_frames` 一个计数器管五条泳道,于是**一次运行里只有第一次拒绝会印出来**——用户那份日志因此只写了 `overlay` 泳道,对真正丢了字的那一页只字未提。现在按泳道记账(`GlyphRefusalLog`),每条泳道第一次变哑都留一行;`BT_PREVIEW_TRACE` 的 `frame … refused=` 本来就按泳道记,两边现在说同一件事。

**红证与门**:`bt-render` 的 `a_full_atlas_takes_the_labels_before_it_takes_the_page`——一帧里 chrome 与页面各铺 24 叠汉字(每叠自己的字号,所以同一个汉字在每叠上是不同的位图),两边**各自都放得下、加起来放不下**;按新次序 `refused=chrome` 而页面那片红墨在读回的像素里,按旧次序 `refused=preview`、页面一个红像素都没有,正是报告那张截图。`a_refused_lane_names_itself_and_leaves_the_frame_owed` 加了第二条泳道,钉住「每条变哑的泳道各留一行」。`bt-app` 的 `a_textless_run_is_asked_for_again_a_bounded_number_of_times` 钉住封顶与清零,以及交换链那两种拒绝不进这本账。

**明写的没做**:**没有去减少一帧要的字形总量**。真正让天花板变远的做法是「看不见的字不进图集」(`shape_chrome_labels` 至今没有 `shape_preview_body` 那道「整个盒子落在 clip 外就不排」的过滤,一个被完全遮住或滚出视野的标签照样铸它的位图),以及聚焦卡那一列按卡片尺寸重铸一整套迷你字号——两件都在这条账上,都留给专门的一片。本片做的是**不够分时怎么分**,不是**让它够分**。**两条都在 §7.1.3m 兑现,而兑现的样子跟这里写的不一样**:第一条做了(`shape_chrome_labels` 现在有那道过滤,红门量到 30 个字的地方原本铸了 720 张位图);第二条**核实下来本来就没有欠账**——聚焦卡的迷你字号是 `FOCUS_MINI_TERM_FONT_LOGICAL_PX`/`FOCUS_MINI_FILES_FONT_LOGICAL_PX` 两档定值乘 DPI,不随卡片尺寸变,而滚出视口的卡在**造内容之前**就被跳过了。而上一段那句「一个 4K、200%、全中文的窗口是够得着这个天花板的」,§7.1.3m 量出来是 **3.3%**,不成立。

**7.1.3m 让它够分:一帧到底问图集要多少,以及那个 8192 是谁的(2026-08-29 续 §7.1.3l;`crates/bt-render/src/glyph_census.rs`、`crates/bt-render/src/lib.rs`、`crates/bt-app/src/glyph_trace.rs`)。** §7.1.3l 结在两句写在明处的欠账上:**没有去减少一帧要的字形总量**,以及那条天花板(「约 2.5 万个位图还过得去,约 3.8 万被拒」)是在一台机器上用手量过一次、**没有第二次量法**的。这一片先造量法,再照量出来的数改。

**① 先量:`GlyphCensus`,四条泳道各报三个数。** 每条泳道本来就是把一串 `TextArea` 交给 glyphon 然后就忘了;现在两条 grid 的 prepare 与那条 chrome 的 prepare 都从**具名函数**取这串(`grid_text_areas`/`status_text_areas`/`chrome_text_areas`),普查走同一串数**同一件事**——一个自己重建几何的普查是关于笔尖落在哪里的第二种意见,而亚像素分格正是由那个几何决定的,两边一旦分家,数出来的就不是被问到的那个需求。报三个数:**instances**(交出去的字形个数,是画的代价)、**unique**(它们落到多少张不同位图上,是图集的代价,键就是 cosmic-text 自己的 `CacheKey`)、**ink**(那些位图铺开的面积,对着设备的 `max_texture_dimension_2d` 平方)。ink 明写是**下界**:glyphon 按桶打包,桶里有废料,真纹理比这个数先满;下界的用处就是下界的用处——光是墨都过了顶,再好的打包器也救不了这一帧。普查**为了量而铸**(问 swash 一个字形的尺寸就是叫 swash 画它一遍),所以除非有人点名要,它一帧都不跑。

**② 量出来的数,和它推翻的那个诊断。** 夹具是报告自己那扇窗:3840×2160 @200%、8 张卡的聚焦列、chrome/终端/文档全中文,汉字取 2500 字常用表大小的池子、以均匀抽样的**平方**做频次偏斜(均匀抽出来的是字典不是文章)。一帧:

```
requested=7361 unique=4515 faces_sizes=4515 bins=1.00x shared=4
ink_px=2227129 roof_px=67108864 occupancy=3.3% side=8192
grid=2160/1230 chrome=186/184 preview=2406/1383 overlay=2609/1722
```

**3.3%。** 再走 240 帧、每帧一整屏**新**汉字、24 档字号轮换——`a_long_chinese_session_never_runs_the_atlas_out_of_room` 一次也没被拒。所以 §7.1.3l 写的「一个 4K、200%、全中文的窗口是**够得着**这个天花板的」**不成立**:一帧的墨被它画在的那面玻璃限着(`present_frame` 自己的合同早就是这么说的),一整扇 4K 窗的中文离 8192² 差一个多数量级,而报告里那次拒绝要么来自**一帧里同时存在的字号数**远多于任何真实版面(那把量天花板的梯子要 24 档字号 × 24 叠汉字才够得着),要么来自这一片没能复现的第三件事。**这条写下来,是因为下一个人不该再从「一帧撑爆图集」出发。**

**③ 看不见的字不进图集。** `shape_preview_body` 从出生起就拒排一个盒子活不过自己 clip 的段落,`shape_chrome_labels` 一直没有那道过滤——§7.1.3l 写下了这件事而没有做。现在做了,`crop_to` 一句,连同它自己那两条拒绝(`NaN`、被调用方裁反了的盒子)。**为什么它是真的省而不是省了个寂寞**:glyphon 是**先**把字形栅格化、先在纹理里给它找到位置,**然后**才拿它去比 `TextArea.bounds`(`text_render.rs`,bounds 的算术在 `try_allocate` 那个循环**底下**);它只白送一个方向的裁剪——`is_run_visible` 会在问任何字形之前丢掉整条竖直方向出界的**行**,横着却没有对应的一招。所以一条被推出自己那栏的行——滚过最后一个 tab 的标签条、跨在视口边上的那行 git 图里的徽章、一个正在飞的 pane 背后 clip 已经合上的说明——把自己的位图一个不少地铸完,再一个不少地扔掉。红门 `a_label_behind_a_closed_clip_casts_nothing` 量的就是这个:30 个字要的地方铸了 720 张。三条在 grid 之后见图集的泳道都走这一个函数,所以省的是 chrome 的、卡片的、菜单的。**聚焦卡那一列不在这条账上**:`refresh_focus_thumbnails` 在**造内容之前**就按列的 clip 把滚出去的卡整张跳过(`focus_thumb.rs` 的「可见性」闸门),`focus_rail_chrome` 与 `focus_card_mini_chrome` 再各拦一道;终端也一样,`prepare_text_rows` 只铺 `frame.drawable_rows()`。两处都是核实,不是新做。

**④ 一份字形多处用:键里没有泳道,也没有颜色。** 图集是整个进程一张纹理,一张位图的身份是 cosmic-text 的 `CacheKey`——face、glyph、字号、字重、flags,以及笔尖落进的那个亚像素格。**没有泳道,没有颜色**,这两个「没有」都承重:界面/终端/文档全中文的窗口来回画的就是那几百个字,一个按面或按墨各留一份的图集会把唯一有天花板的那个数乘起来。红门 `one_ideograph_at_one_size_is_one_bitmap_in_every_lane` 量的是严格答案:同一句话放进 chrome 与一个 overlay 层、两种墨色,是**六十个 instance、六十张位图**;页面那条泳道由另一个函数按另一套度量排,所以对它问的是关于字体的那一问——同样的字同样的字号必须解析到 chrome 那同一批 face,否则两个面连**能不能共享**都谈不上。顺带量到的:夹具的亚像素分布是 **1.00x**——笔尖落在整像素上,那把键允许的十六倍乘数一分没花。PIN `every_lane_prepares_into_the_one_shared_atlas` 把「一张」钉住:**按泳道分图集是买某条泳道自己天花板的那个显而易见的办法,而它是用把每个共享字变成两张位图买的**,所以这条路这一片明确不走。

**⑤ 那个 8192 从来不是机器的数,是我们的。** `wgpu::Limits::default()` 把 `max_texture_dimension_2d` 压在 8192 当可移植性底线,而这个 crate 开的每一个 device 都接受了这个底线——在自 D3D feature level 11 起就做 16384 的硬件上。glyphon 正是照 `device.limits()` 定图集大小、按倍增长、在下一次倍增会越过那个数时答 `AtlasFull`。所以**报告里那个「设备上限 8192」不是 GPU 说的,是我们没要**。改法是问将要授予它的那个 adapter 要它有的那一个数,**只动这一个字段**:要 adapter 的整份 `Limits` 是另一个大得多的承诺(每个 buffer 绑定、每个 binding 数、每个 workgroup 尺寸),按跑过这程序的最宽的机器许下、在最窄的那台上毁约。这台机器上同一个夹具因此从 8192²/3.3% 变成 **16384²/0.8%**,四倍余量,而且不用到就不花:图集仍然从 256² 起,仍然只在必须时才长。红门 `a_device_takes_the_largest_texture_its_adapter_offers`。

**⑥ 天花板现在在实机上也量得到。** `BT_GLYPH_CENSUS` 指一个文件,窗口每逢**这一帧要的东西变了**就写一行(静止的窗口一行,不是一秒六十行),内容就是上面那一行的字段。默认关,而且必须关:量一张位图的尺寸就是把它再画一遍。

**明写的没做**:**多页图集没做,因为量出来不该做**。这一片的触发条件是「①–④ 之后仍超过单张的 70%」,量到的是 0.8%;而多页(或按泳道分)会把④那条「一份字形多处用」拆掉,拿确定的损失换一个量不出来的收益。真要做的那天,先复现一帧**确实**顶到 roof 的实机 census,再动。第二件当时写作「报告那次拒绝的真实成因仍未复现」,**下一段把它复现并结掉了**。

**真因:图集满的不是面积,是货架——而 glyphon 从来不会重新排货架(2026-08-29 续 §7.1.3m;`crates/bt-render/src/lib.rs`)。** 上一段留了一句「报告那次拒绝的真实成因仍未复现」,这一段是它的答案。**结论一句话:一帧 3.3% 与「图集满了」两件事都是真的,因为图集里用完的从来不是面积,是几何;而一旦用完,glyphon 没有任何一条路把它还回来——那次拒绝是这个进程余生的每一帧。**

**① 逐出是好的,归还不是。** glyphon 的 `InnerAtlas::try_allocate` 在 `packer.allocate` 失败时**先**按 LRU 逐出、逐不动了才去 `grow`,所以「到顶了就不再逐出」这条猜想是假的(`text_render.rs` 里 `grow` 在 `try_allocate` 的**外**层循环,`try_allocate` 自己的循环里就有 `pop_lru`)。`glyphs_in_use` 也是干净的:它是**整张图集**一个集合、不按 `TextRenderer` 分,`TextAtlas::trim` 每帧清一次、而且清在 `present_frame` 每一条出路之外(§7.1.3l 已经把它挪到那儿),所以「某条泳道这一帧不 prepare,它上次标的在用就留着」这条猜想同样是假的——一条不 prepare 的泳道什么也没标。**问题在逐出之后**:glyphon 底下是 `etagere` 的 `BucketedAtlasAllocator`,一个**按高度分级的货架打包器**,而它的空间不是可加的数量,是**几何**——一个 bucket 要等它里面**每一个**字形都走了才把宽度还回来,一层货架要等它是**最顶上那层**(或连着的、至多三层、同一列的空货架)才会被回收。于是:一场按**一梯字号**画字的会话——zoom 的每一档、文档的每一级标题、chrome 对正文对迷你卡——铺下几十种高度的货架,再让每层上都留一个「这一帧还在画」的字形把它钉住;到某一刻,一张高的位图**哪儿都放不下了**,而图集另有一半是空的。`grow` 此时已经在 `max_texture_dimension_2d` 上,答案就是 `AtlasFull`。

**② 它是永久的,而「重试」重试的是同一个问题。** §7.1.3l 给 `PresentedWithoutText` 写的收敛论证有两半:「一帧的字被它画的那面玻璃限着」是真的(§7.1.3m 量到 3.3%),「trim 把上几帧留下的占用放开,所以下一帧就有地方」是假的——trim 清的是**在用集**,而用完的是**货架**。两次尝试之间**没有任何东西变过**,所以第二帧在同一个地方被同样地拒绝,第三帧也是。红门量到的正是这个:同一个夹具、同一台机器,不修时第一次拒绝落在第 191 帧,之后**连着十帧**一个字都没有;把同一场会话放长到四千帧,**2015 帧被拒,其中一段连续 1122 帧**画着一页没有字的文档——而它最忙的一帧只要 roof 的 **19.8%**。用户那份日志里跟着响的 6.36 秒挂起看门狗,就是这段里窗口以最快速度重画同一张缺字的画。

**③ 修法:一帧丢了字,给它一副新货架,而不是再看一眼旧的。** `GpuContext::close_the_frame` 现在是一帧对共享图集的**唯一**收尾动词,按这一帧自己的判决分两路:平常的一帧照旧 `trim`;**丢了字的一帧换一张新的 `TextAtlas`**——也就是一副从 256² 重新长起来的新货架。图集里的位图是**纯缓存**(每一张都只是一次 swash 调用),所以丢掉的只有再画一遍的时间;而那一帧本来就已经欠着、已经被 `PresentedWithoutText` 要着重来——**这才让重试真的是重试**。**它不是「定期清空」**:没有计时器也没有水位线,只在一帧**已经**丢了字的那一刻动手,那正是这份缓存被证明不再是缓存的唯一时刻。**而且一次拒绝只换一次**:`glyph_atlas_refitted` 这个闩没被一整帧的字清掉之前,第二次拒绝不再换——一副**上一帧才新做的**货架还是放不下,那是另一种病、另一个名字:这一帧要的东西比这台设备能做的最大纹理还大,重排货架治不了它,而每帧重排会把一扇只是装不下的窗口变成一扇每秒把全部字形重铸六十遍的窗口。闩由第一帧**完整落地**的帧清掉。**每一次重排在 `diagnostics.log` 里留一行,带序号**:`Folio glyph atlas was repacked after a textless frame (refit #N)`,与那条拒绝行同一族口径(`Folio glyph atlas …`),一次 `grep` 找齐两半。**为什么非要这个数**:重排本身在屏幕上是看不见的——下一帧就把字画回来了——而分辨两种病的从来不是事件是**频率**:几百帧一次是货架按它必然的样子磨完了、修法在工作(红门量到四千帧八次),一秒一次是这一帧根本装不进这台设备最大的纹理、重试在空转。计数与那句话由**同一个语句**铸出(`note_a_repack`),`glyph_atlas_refits` 全 crate 只有这一个写入点,所以「一次重排=一行」是代码的性质而不是关于代码的承诺;序号取自自增之后,所以一次运行的第一次重排是 `#1`。安全性:换图集会让所有已 prepare 的坐标失效,所以它做在 `present_frame` 画完并 present 之后,而每条泳道的 draw 都由**这一帧自己的 prepare** 算出来的那个 flag 把着——这一帧没 prepare 的泳道不会被 issue;`TextRenderer` 的 pipeline 由共享的 `Cache` 按 format 铸,format 不变,所以渲染器本身仍然有效。

**④ 日志写的是 overlay,而丢字的是「预览正文」——两句话说的是同一条泳道,不需要第二个毛病。** 两条都要说清,因为读日志的人两种版本都可能碰上。**其一,一张浮起来的卡片,它的正文就在 overlay 泳道里**:`compose_frame` 的 overlay 段对每一层都做 `layer.body` → `shape_preview_body`,与页面那条泳道**同一个排版函数**,只是交给这一层自己的 `TextRenderer`。所以「悬浮卡/浮窗上那一页正文一个字都没有」在日志里天然就叫 overlay,而它上面那行 pane 头(chrome 泳道)留着字——报告描述的画面正是这个形状。**其二,如果那次运行还是 §7.1.3l ③ 之前的版本**(`glyph_degraded_frames` 一个计数器管五条泳道,判据是 `== 0`),那么**整个进程只有第一次拒绝会印出来**,日志说的是「这个进程第一次变哑的是谁」而不是「这一帧丢字的是谁」,后面每一帧的 preview 拒绝一行都没有。两种读法都不需要 preview 有毛病,而 preview 也确实没有第二种毛病:**没有「被逐出后没重铸」这回事**——一条泳道每帧把自己那批字整批重新 prepare,`AtlasFull` 让**整批**失败(glyphon 在第一个放不下的字形上就返回),从来不是漏掉几个字。

**红证与门**:`a_session_long_enough_to_wear_the_packer_out_gets_its_text_back`(`bt-render`)。夹具量的是**比值**不是数量,因为病读的是比值:一扇 1600×1000 的窗对 2048² 的 roof 约占五分之一——与一扇 4K 窗对这台机器真有的 16384² 在同一个刻度上,而**一帧从来不是放不下的那个东西**,门里那条 occupancy 断言就是把这句写在明处;十级字梯 × 2500 字的池子是图集能装下的约十倍,所以整段跑下来打包器一直在逐出压力下。roof 由一道新的测试门 `GpuContext::headless_under_a_texture_ceiling` 压低——`required_limits` 是「至多这么大」,压低它拿到的是**和一台适配器本来就只给这么多的机器一模一样的那个数**,买到的只是时间:把真实会话几天走到的状态压进一分钟。**断言的是复原不是完美**:拒绝仍然会发生(那是打包器在说它磨完了,没有调用方能预见),不许再发生的是报告里那个状态——**下一帧在同一个地方再被拒**。所以门是「没有两帧连着丢字」加「拒绝是绊一下而不是这场会话的常态」,再加一条「每一帧丢字都对应一次重排、而重排从不落在没丢字的帧上」。

**明写的没做**:**没有把这条病根本身修掉**——`etagere` 的货架不会被整理,`vendor`/registry 只读,这一片改的是我们这一侧的收尾动词。真正让重排也不必发生的做法有两条,都要动上游或换打包器:一是让逐出**认高度**(逐出的是能腾出那一层的那些,而不是全局最久没用的那个),二是一个不按货架分级的打包器。两条都要先有一份实机 census 证明重排的频率值得——这台机器上量到的是四千帧八次,一次约五百帧,而每次的代价是一帧的字重铸一遍。
...
**⑦ 预览里的一条链接读的是同一张表(用户裁决 2026-08-29;`web_address_activation` / `preview_link_activation` / `preview::LinkAction::Web`)。** 由头就是 ⑥ 自己上一段记下的那笔挂账,一个字不用改地成立:预览里 markdown 正文的 `http(s)` 链接走 `preview::link_action` → `LinkAction::Browse` → `shell_execute`,**平点直接交给系统浏览器,而且那条路根本不读 `control`**。它写在 2026-08-13,比 `ClickIntent`(2026-08-20)早四个月,于是当终端那一侧把这个手势定成「平点 = 留在窗内、`Ctrl`+点 = 交出去」之后,**同一个产品的另一张面对同一个手势给的是反过来的答案**——而这正是 `ClickIntent` 当初被铸出来要终结的那种自相矛盾,只不过它换了一张没人回头看的面重新长了出来。⑥ 没有把它算进自己的报告,理由是「它不是『什么都不发生』」;但它当时就把裁决预告完了:「**下一次碰预览链接时按同一张表改**,不要在那边另写一个 `if control`」。这一片就是那一次。

**裁决:预览里我们自己画的文档正文中的 `http(s)` 链接,平点 = 在本窗开出这一页,`Ctrl`+点 = 交给这台机器;`file:`/相对路径那一臂原样不动。** 前半句是 ⑥ 的原话搬过来,连门都是同一扇:平点先过 `webnav::address_bar`,`Navigate` 交给 `open_web_page`(它自己的合同就是「每个调用点先过这扇门」),**被拒要出声**。后半句是 2026-08-13 那条裁决继续有效,并且**它这一臂不读修饰键**:`Ctrl` 在这里没有第二个目的地可换——「指到文件=预览它」本来就已经是「在这扇窗里发生的那个目的地」,读者指的是一份挨着当前这份在读的文档。所以这不是给「Ctrl = 交出去」开的口子,是那条总规矩的主语在这一行上本来就只有一个值。

**修法:把 `http(s)` 那一行从表里抽出来,让两处调用它而不是各自抄一遍(`web_address_activation`)。** 这是本片唯一的结构动作,也是唯一能让「同一张表」这句话变成事实而不是承诺的写法:**这个程序里现在有且只有一处地方,一个修饰键和一个网址会碰面**。`hyperlink_activation` 的 `http(s)` 臂变成对它的一次调用加一次改写(`Page` 要带上 uri、`Browser` 不带),预览侧的 `preview_link_activation` 则是一次**组合**而不是第二张表:`link_action` 答「这串字是哪一类东西」(文件 / 网址 / 什么都不是),网址那一类交给上面那一行读。`link_action` 自己的 `Browse` 因此改名 `Web` 并**降级为解析器**——它原来的名字就是它原来的病:一个叫「浏览」的返回值,读它的人不会想到还要问修饰键。

**这一面上到底有哪些链接,查清写下,而不是「大概就 markdown 吧」。** 本窗自己画的预览面里,**只有 markdown 正文会产出可点的链接**:`PreviewLinkSite` 只在 `PreviewDocument::Markdown` 这条臂上被造出来,源码视图(`.html` 的源码也在内)、diff、表格、hex、Empty 全都不产;「无法预览」那张缺席卡的出口是它自己的 `Open in default app` 钮,不是一条链接。**一瞥卡(`PreviewSurface::Peek`)是同一条流水画的,链接也照量**,但它不在 `preview_surface_at` 里,于是指针既不会在它上面点亮一条链接、也不会戴手形、更点不到——**它没有许过承诺,所以这里没有谎要拆**;哪天这张卡接上指针,它接的会是这同一张表,因为它走的本来就是同一个 `open_preview_link`。

**WebView2 渲染出来的 `html`/`pdf` 页面里的链接不在本片之列,写明。** 那是引擎自己的导航,不是本窗画的记号:它有自己那道门(`webnav::Origin::NavigationStarting`,以及 W2 ③ 用 `is_loopback_host` 划的「一张页面自己的链接去哪里」),两条路管的是两件事——本片管的是**本窗画出来的一条链接被按下时去哪儿**,那条管的是**一张页面自己要求跳转时准不准**。把它们并成一条会让「平点/Ctrl」这个手势去管一个根本没有手势的场合。

**被拒出声,但出声的地方是这张面自己的嘴。** 终端那一半是 hover 行加 `· blocked`(`show_blocked`),那条行之所以能说话,是因为地址**印在格子里**、有一段格子可以画下划线并挂一行状态;预览里的一条链接**没有格子**,那条路在这张面上是哑的——把一个假的 `HyperlinkHit` 递给它,只会得到一次什么都不画。所以这里走的是本窗对「这件事刚刚发生」早就定下的那扇门:2026-08-16 的通知裁决,`Runtime::toast`,**卡片站在动作发生的那张面上**而不是窗角。**文案一个字都不新造**:说的就是终端那行被拦时说的两件事——地址,和 `· blocked`(`HyperlinkBlockedSuffix`)——而且经过同一条 `printable_address`,因为一个来自外面的地址里可能带控制字符,而**「一个地址怎么被印出来」必须只有一个答案**,不能一张面一个。判断在一处(`Runtime::open_web_address_here` 里那一次 `address_bar`,两张面都调它,答 `false` 就是被拒),嘴有两张,这是对的分法:两张面各自只能替自己说话,但它们不许对同一个字符串给出两个判断。

**手形跟着同一处读数走,而它这次是从一个更弱的读数换过来的。** 原来是 `preview_link_hover.is_some()`——**这是下划线的那句话,不是手指的那句话**:一条 `mailto:` 写在文档里,指针照样戴手,按下去什么都不发生,这就是 7.1.5f 那句「一个只答 hover 不答 click 的记号等于本窗在对自己画的东西说谎」原样落在预览上。现在 `preview_link_grasp` 问的是 `preview_link_activation`,与按下花掉的是**同一个表达式**;于是 `mailto:` 不再戴手,而一条网址**两种修饰键状态下都戴**,因为这一行的两半都有事发生。下划线不动:它说的是「这里有一条链接」,那是关于文档的事实,与手势无关——两句话本来就不是一句。`ModifiersChanged` 上重算光标那一条已经在,不用新写。**预览这张面没有状态行,所以 `ControlClickHint` 那句 `Ctrl` 旁白在这里没有地方站**;这是差异不是欠账:那句旁白是为了给「手形一样、去处不同」补一个投影,而它要的那条已经在画的行,预览里根本不存在。

**`control` 在按下时记住,不在抬起时重读。** 与终端的 `SelectionDrag::hyperlink_control` 同一条理由,一字不差:一次点击中途松开或按下的 `Ctrl`,不许把一个已经开始的手势的去处换掉。所以它记在 `PreviewTextDrag::control` 上,和链接本身、和那把 `DragLatch` 放在一起。拖拽仍然只是选择:`preview_press_opens_its_link` 一个字没改。

**红门 `a_link_inside_a_preview_reads_the_same_table_as_a_link_in_the_terminal`。** 四条地址(报告里那条 `claude.ai`、一条 github、一条 loopback、一条大写 scheme 带 query 与 fragment)在**两张面上被问同一遍**,断言它们答得一样;再断言平点这一臂交出去的字符串**那扇门收**;再断言文件臂在两种修饰键下都是 `Preview` 且相对路径仍按文档自己的目录解;`mailto:`/`#section`/空白两半皆哑且不戴手;`http://` 这种自己就不成立的地址平点哑、`Ctrl` 出 `Blocked`;最后钉死**被拒这条路真的够得着**——`https://claude.ai@evil.test/code` 文本合格(所以平点确实走到 `Page` 臂、确实提了要求),而 `address_bar` 认它是钓鱼形状并拒绝,于是「被拒出声」是本窗必须做的事而不是永远到不了的分支。变异实测:把 `LinkAction::Web(url) => PreviewLinkActivation::Browser(url)`(2026-08-13 那个不读修饰键的动词)放回去,**每一条平点行全红、每一条 `Ctrl` 行全绿**,红出来的第一句正是这笔挂账的原形:`left: Browser("https://claude.ai/code/artifact/…")` / `right: Page(…)`。
...
**2026-08-27 再多一根:seam(见 §7.30;2026-08-28 两次复议,判词以 §7.30 为准)。** 一个候选词里,凡「一个**接缝分隔符**(ASCII 标点中路径从不用来拼写的那些,`is_ascii_punctuation()` 减去 `/ \ . - _ ~`)紧跟一个非 ASCII 字符」的位置都是一道接缝;**ASCII 开括号(`(` `[` `{` `<`)是接缝点,并且不看后继字符**(2026-08-28 晚)。每道接缝产出一种**更短的读法**;几种读法**从长到短**去问磁盘,第一个在的算数,整串在则整串赢。本表 23 行的字面判决**一行没改**——第 16 行(ASCII 句点)与第 17、19 行(非 ASCII 标点)恰恰是接缝**不**成立的那两侧,而第 18 行的行号与接缝共用一个冒号却永不同时成立。新增十四行(40–45 为 2026-08-27,46–49 为 2026-08-28 下午,50–53 为当晚开括号与它划出的边界)在 §7.30。
...
- **ASCII 开括号是接缝点,不看后继字符(2026-08-28 晚,用户验收 next16)。** 这是本条规矩里**唯一**不问后面跟着什么的那个标点。现场:`dist\folio-next16.exe(0.1.0 (84d843f47e))`——`(` 后面是 ASCII `0`,类转换不成立,候选词一路吃到 `)` 才停,拿 `dist\folio-next16.exe(0.1.0` 去问磁盘、答「不在」,而那个文件真的在。版本号横幅、`(1)` 副本后缀、`docs/a.md(说明)` 三者的共同点不是括号后面那种文字,**是括号本身**:它开的是一句关于刚才那个名字的旁白,不是那个名字的下一段。类同样是一句话而不是一张表——**开括号 = `is_closing_delimiter` 里那几个 ASCII 闭括号的另一半**(`)` `]` `}` `>` → `(` `[` `{` `<`),四个都不是 Windows 路径的合法字符,所以名字读到它为止一点也没少(`is_ascii_opening_bracket`)。**开括号是接缝、不是终止符**,这是整条裁决的分寸:终止符会让 `D:\x\a(1).txt` 连问都问不出去,而接缝只是多产出一种更短的读法,并且只在磁盘否掉更长的那一种之后才轮到它。**开括号入类当晚随即画了一条边界(同日裁决,边界表第 53 行):接缝切出来的读法**不许以路径分隔符结尾**——`src/{old => new}/main.rs` 的 `src/` 是目录前缀而不是名字,所以 git rename 压缩那一行回到「什么都不认」。一句话:**一种读法的最后一个字符必须是名字的一部分**。
- **ASCII → ASCII 不是接缝。** `D:\x\a.md,b` 是**一个**名字;逗号在 Windows 文件名里合法,而这一行上没有任何东西说这一个是标点。(**开括号除外**,见上一条:`docs/a.md(1).txt` 现在也切出 `docs/a.md` 这一种更短的读法——切出来只是被**问**,不是被**画**,整串仍然先问。边界表第 42、48 行。)
...
**④ 代价:多出来的 stat 只在候选词含接缝时发生,而且搜索是有界的。** 预算纪律(§7.1.5j ⑥、§7.1.5k「预算纪律一个字没动」)原样:I/O 仍只在 worker 上,一帧 256、一 pane 512、账本 4096 三个常数一个没动,答案照旧两种都记。没有接缝的 token 一个额外的问题都不产生(`prose_seam_ends` 返回空 `Vec`,不分配)。**词法这一侧还有一道可证等价的短路**:一条 token 既没有非 ASCII 字符、也没有开括号,就一道接缝都没有(**2026-08-28 晚随开括号一并改写:原来只问 `is_ascii()`**)——这个事实与 token 的末端一起被缓存(每个 token 走一趟 `token_may_carry_a_seam`,两个问题合在同一趟字节里问,token 内每个开口只读一个 bool),于是一屏普通程序输出上,本片的代价**恰好是零**。**另有一条证明而不是一次调参**:**裸开口**的接缝搜索停在 `bare_form_limit`——路径字符的那一段,加上后面可能跟着的 `:line[:col]`——因为再往后的任何一种读法,它的路径那一半必然含着 token 里第一个非路径字符,而 `bare_candidate_opens_at` 一定拒绝它。于是那条「开口测试给这个循环买下的界」原封不动(一行没有终止符的长文本仍然不会被逐字符重读一遍)。**带锚的**(`./`、`../`)与**盘符开头的** token 搜索整条,理由与它们本来就能整条 `release_prose_tail` 一样:这两种开口和盘符前缀一样稀少。实测(`a_full_screenful_is_scanned_in_a_time_worth_printing`,本机 2026-08-27,机上另有 agent 在编译):密屏 274 µs/帧(改前)→ 215 µs/帧(改后)、散文屏 6.9 → 6.3 µs/帧——两者之差在这台机器的噪声带之内,而结论不靠这两个数支撑:ASCII 短路让那一屏根本走不到新代码。**2026-08-28 晚重测(开括号入类 + 分隔符边界,release;基线 `f60d988` 单开一个 worktree,两边交替各跑九轮,取每边的**最小值**——机上另有两条线在编译,而竞争只会往上加时间,所以最小值才是最接近真实代价的那个数):密屏 166.5 µs/帧(改前)→ 167.6 µs/帧(改后,+0.7%)、散文屏 1.549 → 1.585 µs/帧(+2.3%)。两边的单轮跨度都比这点差大得多(改前密屏 166.5–267.1,改后 167.6–290.0),散文屏更是有两轮改后比改前快——**这两个数说的是「量不出退化」,不是「量出了 0.7%」**。结论仍然不靠这几个数支撑,靠的是下面那条界。** 开括号把短路的适用面调小了(密屏那一行里就有一对括号),而**界没有变**:裸开口的接缝搜索仍停在 `bare_form_limit`,而两个开口的那两段路径字符**永不相交**——一个开口的前一个字符按 `candidate_start_boundary` 的定义不是路径字符,所以一行仍然只被读一遍,不是每个字符一遍。
...
**⑨ 应用自己折断的下半截也先切接缝再拼(2026-08-28 与 §7.1.5k ② 共存;当晚随开括号一并改成从长到短)。** Claude Code 把 `dist\folio-next15.exe` 折在它**自己**的换行上,并给下半截尾巴焊了散文:`5.exe(反斜杠)`。`rejoin_across_newline` 让下半截交出**同一个 token 的几种读法**——整条在前,每道接缝一种——再**从长到短**逐个拼、逐个问磁盘,与单行那一侧逐字相同。**必须是从长到短而不是一刀切在第一道接缝上**,否则开括号这条新规矩会在折行处变成终止符:`a(1` + `).txt` 会被切成 `a` 再去问,而整串根本没被问过。没有接缝时这仍是整条 token,与从前逐字节相同;拼不出恰好一个引用的那一种读法交给下一种,全都拼不出则第③闸拒绝、上半截由末列截断门压住——是空白,不是错的链接(`a_wrapped_lower_half_that_carries_a_seam_rejoins_the_name_alone`)。一个**没人看过**的名字结束搜索而不是交给更短的一种,因为更长的那一种还没有答案(②)。第⑤闸(两半各自单独都不是已验证引用)问的是**两半**而不是拼出来的那一串,所以它站在这几种读法前面、只问一次。

**边界表(§7.1.5j)补十四行(40–45 为 2026-08-27,46–49 为 2026-08-28 下午,50–53 为 2026-08-28 晚的开括号一条与它当场划出的边界,并就地推翻第 48 行)。** 与前面 39 行同一张表的续号;每一行一条测试。
...
| 48 | `docs/a.md(1).txt`(ASCII 括号) | `docs/a.md`(**2026-08-28 晚推翻**,原判「不切」) | 当日下午的判词是「`(` 后面是 ASCII `1`、不是转换 → 接缝不触发」;当晚 next16 的现场推翻了它——开括号不看后继字符(第 50 行)。切出来的是一种**被问**的读法而不是一句承诺:整串仍然先问,`docs/a(1).txt` 真在盘上就整串赢;它不在而 `docs/a` 在,就认 `docs/a`——**存在即事实**。裸拼法里整串本来就不成候选(`)` 是闭合括号、第 6 行),那一侧从来只有这一种读法 |
...
| 50 | `dist\folio-next16.exe(0.1.0 (84d843f47e))` | `dist\folio-next16.exe` | 用户 next16 验收的原句(2026-08-28 晚)。开括号后面是 ASCII `0`、类转换不成立,所以这一行正是开括号自己那条规矩的现场;`(` 不是路径字符,裸拼法整串不成候选,接缝是这一行唯一的读法 |
| 51 | `见 D:\dist\x.exe(0.1.0` | 先问 `D:\dist\x.exe(0.1.0`,再问 `D:\dist\x.exe` | 绝对拼法里整串**是**候选,所以两种读法都产出、从长到短问,代价是这一个 token 多一次 stat;第一帧两个都没答案时一条都不画(与第 44 行同一条纪律) |
| 52 | `src/app.ts(7,19): error TS2322`(tsc/MSVC 括号位置) | `src/app.ts`(**新认**) | **开括号这条规矩的连带**,原「不认风险」场景 7、8 判「整项不画线」。画的正是场景自己写的「只画路径」,只差一个还没有契约的 `(12,34)` 片段;场景禁止的那种前缀(`…main.cpp(12,34`)一个都没被产出过 |
| 53 | `src/{old => new}/main.rs`(git rename 压缩)、`cd docs/`、`docs/` | **什么都不认**(场景 44 原样保住) | **一种读法必须以「名字的一部分」结尾**(2026-08-28 晚裁决)。花括号确实是接缝,但它前面那一种读法是 `src/`——一个**目录前缀**,是没人写过的名字;磁盘上有也不认,因为「存在」从来只是**画**一条引用的执照,不是**发明**一条的执照。这一条**补的是第一条拒绝的漏**而不是在旁边新加一条规矩:裸引用是靠**带着分隔符**才被收下的(那是普通散文没有的那一个记号),而**结尾**的分隔符恰恰是这个记号唯一不证明「有两段」的地方——`docs/` 就是 `docs` 后面加了一条斜杠,而 `docs` 单独出现从来就不是引用(第 1 条拒绝、`is_relative_reference`)。绝对拼法**不加**这一条:那边分隔符从来不是「这是引用」的证据(盘符前缀才是),而 `D:\` 是一个真地方、它自己的拼法就以分隔符结尾 |

**红门与红证(先写、跑红、贴原文,再改)。** 十一条在 `bt_transcript::paths`(第十、十一条为 2026-08-28 晚的开括号与它当场划出的边界),另有一条断言加在 `bt_term::inline_image` 的既有测试里(⑦):
...
- `an_opening_bracket_is_a_seam_whatever_follows_it` —— `left: [] / right: ["dist\\folio-next16.exe"]`(2026-08-28 晚;用户 next16 那一行,词法层一个候选都没有)。
- `a_reading_that_ends_on_a_separator_is_not_a_name` —— `left: [("src/", "file:///D:/case/src")] / right: []`(2026-08-28 晚;开括号入类当晚立刻长出来的那条边界)。
...
**这条裁决改判了两条既有测试,每一条都是它自己的判词被推翻而不是被绕开。** `a_bracket_glued_to_a_cjk_word_seams_like_any_other_separator` 里第 48 行那句断言(「an ASCII `(1)` must not seam a name into a shorter one」)正是本条推翻的那一句;`group_a_quoting_provider_prefixes_and_position_syntax` 的第 7、8 行按边界表第 52 行改写。`group_d_git_and_virtual_schemes` 的第 44 行**一字未动地保住了**——它一度被开括号带走,同晚的第 53 行又把它接了回来,而接回来的办法是补上一条关于「名字怎么结尾」的规矩,不是给 git 语法开一个特例。两条改判都留着理由写在测试里,好让下一个读到它们的人知道旧判词去了哪儿。

**日期:2026-08-27 用户实证,当日落地;2026-08-28 下午改「类」+反引号;同日晚 next16 验收再加开括号一条(并推翻当日下午的第 48 行),均当日落地。**
...
- **谁跟着一起变好了。** `tooltip::wrap` 的读者有四家,这次修的就是它们:设置页的行说明(`settings::wrapped_description`,三行门量的就是它)、会换行的提示条(`main.rs` 里 `TipFace::wraps()` 那一支)、toast(`toast.rs`)、Git 图的提交气泡(`git_graph.rs` 的正文与 meta 两段)。`restore::wrap`/`wrap_anywhere` 的读者有五家,它们本来就断得对,现在只是改读同一张表:恢复提示的 sub 行、PSReadLine 邀请的正文与理由(`main.rs`)、websheet 的 say 行(`main.rs`)、Git 面板空态句(`git_panel.rs`)、缺席卡的细节行(`seats.rs` 与 `main.rs`)。**九个面,一张表。**
...
**而「画在第 i 层的底之上、填充之下」这句话,对一个洞是对的、对一张画是错的,同一个错这一片犯了两遍(2026-08-28 两次实机拍到)。** 一个宿主自己的**体井**就是那些填充之一,所以把 stage 指向宿主的脸,等于把录像画在正要显示它的那张脸底下。浮窗先中招(窗画了自己的脸、填充盖住录像,只有底盘退场那一瞬间画面才露出来),改法是在每扇窗的脸**正上方**推一层**空层**、`float_video_level` 指它;卡随后中招,而且是在浮窗被修好的同一天写下的——卡拿的是 `below_the_file_peek()`,也就是**卡自己那张脸**的下标,而卡的圆角图片井正是它的填充。卡上那张静止图盖在最上面,把这个缺陷藏了两轮(⑤ 的补记)。机制一模一样:`file_peek_layer` 照 `float_layer` 的样子接一个 `below`、自己写下标、推一层空层,播放圆片与控件条**再往上**排(一层先画自己的 quad,再画它托着的画)。**一个 stage 下标必须指向 z 序循环真的会走到的那一层,而那一层存在的意义就是被走到**;它不花 buffer、不花 draw call、不花一枚 quad。红门 `a_recording_on_an_overlay_host_is_drawn_into_a_layer_of_its_own`——门写成**对宿主集合的规矩**而不是对某一个宿主的陈述:每一本「录像画进哪一层」的账,写下的下一行必须是推一层空层。
...
**静止帧当帧撤走**,三个表面各撤各的:pane 和浮窗走 `refit_preview_picture` 那条早退(和一段正在播的录像同一条),卡走 `file_peek_layer` 里把 `picture` 置 `None`。**而卡那一条当初只问了动图这一半,实机拍出了另一半的代价(2026-08-28)**:卡上自己的 ▶ 按下去引擎真开了、条真的在数 `0:01 → 0:06`,画面**一动不动**——八张相隔 700ms 的连拍在画面那块矩形上逐字节相同,读的是十分之一处那一帧的静止图(`clock.mp4` 烧进去的秒表读 `12`),盖在一段正在底下好好播着的录像上;同一个引擎一个手势之后到了浮窗里,读 `9` 并且在动。卡的静止图不走 `refit_preview_picture` 拒绝的那条图片道,它是交给 `file_peek::layout` 由卡自己的**图标**道画的,而图标道排在视频道**之后**——所以留着就一定盖在上面。改法是两半一起问,和这扇窗别处一模一样(`surface_is_playing_a_video(surface) || animation_running_on(surface)`):**「这个盒子里的画在不在动」是一个问题,只是有两种为真的方式**,一条只知道其中一种的闸门,对另一种每次都会答错。红门 `a_card_withdraws_its_still_for_a_recording_the_way_it_does_for_an_animation`(两半都在那句里,且那句真的是 `match` 的闸门)。两者是同一个文件、同一个尺寸、同一个盒子,而视频道整整早一趟——留着的那一张会被画在动的那一张**上面**,而盖在上面的那张是不动的那张。
...
**还有一扇门,这一条规矩自己问不出来:关掉的浮窗(2026-08-28 实机)。** `preview_surfaces()` 的浮窗那一半是从 `float::FloatHost::drawn()` 来的,而 `drawn()` **故意**留着正在退场的窗——它必须留,因为窗在淡出期间还在被画,把它的**视图**在它底下退役等于把退场动画正在动的那个东西抹掉。于是一扇被关掉的浮窗在整个退场期间都还在"活着"的名单里,这条 sweep 无从察觉。**而解码器不是视图**:画面接着解、接着画,底盘已经从它上面走了,一直到退场走完——声音也是。所以浮窗那一臂问的是 `float::FloatHost::live()`(`drawn()` 减去已经 dismiss 的那些),并且排在读 `alive` 的兜底臂**前面**,因为 `match` 取第一条合得上的臂。停在按下那一刻,而不是停在动画末尾。别的路都不用改:pane 换文档、卡的指针移到下一行、tab 关或撕走(座从 `preview_seats()` 里消失)、整扇窗关掉(`WindowRuntime` 析构 → `VideoSeats::Drop` → `shutdown_all`)。红门 `a_closed_window_stops_the_recording_it_was_showing`(①两张名单之间的缝是真的:dismiss 过的窗仍然 `drawn()` 而不再 `live()`,所以这条臂是必要而不是冗余;②sweep 问的是 live 名单并且先问)。

**还有一半在玻璃上,和 ⑫ 是同一个错犯两遍、隔一行(2026-08-28 拍照发现)。** 解码器停了,**画面还在**:关掉的浮窗把最后一帧留在原地——没有头、没有条、周围没有窗,就是一块画——留了三秒半,直到一张 hover 卡收掉、`refresh_preview_for_layout`(它是无条件问的)跑了一趟才扫走。四张相隔 700ms 的连拍在那块矩形上逐字节相同,所以那确实是一张照片而不是一段视频。根因在 `advance_strip_animation`:`refresh_video_layers` 被一道「什么都没在动就别重建」的闸门挡着,而「什么都没在动」当初读成*本窗既没有录像也没有动画*——可 `sweep_video_seats` 就在上一行,**把最后一个座拿掉正是让这句话变真的那件事**,于是唯一需要告诉渲染器「名单空了」的那一拍,恰恰是不告诉它的那一拍。改法保住闸门本来的用处(闲窗一分钱不花),只是**连渲染器一起问**:`|| !self.window.renderer.video_layers().is_empty()`——真正的闲态是「没有在动 *而且* 渲染器手上也没有东西」,代价是一次切片 `is_empty()`。红门 `the_tick_that_empties_the_picture_list_still_hands_it_over`(条件里必须有那一问,且 sweep 必须排在闸门之前——否则是另一个缺陷)。
...
**卡片给的是一个此刻能做的动作,不是一句诊断。** 文案规矩是屏幕上的一句话要说「现在能做什么」,所以策略这张卡的第三句是 `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned`——**`CurrentUser` 是量出来的而不是挑出来的**:同一台干净机上,未提权的 shell 跑 `-Scope LocalMachine` 静默失败(策略纹丝不动),`-Scope CurrentUser` 才真的落下,而一张让读者去跑一条他的 shell 会拒绝的命令的卡片,是把人送到了比原地更坏的地方;`RemoteSigned` 而不是 `Bypass` 或 `Unrestricted`,因为它是能加载这个模块的最弱策略,对从网上来的脚本仍然要签名。**动作句排在路径前面,这是六行上限唯一要付的代价**:`TOAST_MAX_LINES` 把放不下的丢掉并画一个省略号,而这张卡是本产品最长的一段字——重定向过的 `Documents` 下的模块路径自己就能有一百个字符。所以被切掉的那一半必须是最不能拿来做事的那一半:在「这条命令能修好它」和「文件本该去哪儿」之间,读者丢得起的是后者。红门 `the_way_out_survives_the_cards_six_line_cap` 把这条钉住,**两半都有牙**——把路径句放回前面,顺序断言先红;再把顺序断言拆掉,布局断言接着红,卡片被切在 `C:\Users\a-rather-long-` 上,那条命令一个字都没上屏。**行下说明不加动作**:那一行只有一行的位置,它说的是这台机器是什么状态,而一条命令不是状态;它也不再重复「不会加载未签名的模块」那半句,因为按下去的那个下拉正上方就写着。
