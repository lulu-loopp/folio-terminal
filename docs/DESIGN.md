...
**延写(2026-08-27,§7.27):一份本机文件现在也可以被一只手从终端里一次抬走。** 本节立的是「一条本机路径如何成为一个可以被打开的对象」,而那一片把同一条路径在**手停在它上面时**的答案补齐:悬停出卡(页类走 §7.7 ⑥′ 那两条道,一字未改),拖头六个像素则直接开成一扇固定预览浮窗——**走的是 `open_preview_onto`**,也就是本节那扇受控 `file:` 入口自己,所以一张 `.pdf` 卡拖出来的浮窗里坐的仍是引擎上的那一页、头上仍是本节那条地址轨,而不是第二条打开方式。
...
### 7.27 一个文件,不论从哪指向它,都是同一张卡(hover 卡族统一,2026-08-27 用户提议并裁,已落地;`crates/bt-app/src/{main,file_peek,float}.rs`)
...
**⑥′ 一张卡的头只在它真有一扇窗可变时才是把手,而这不是对裁决的折扣,是架构自己早写下的一句话。** 落地时撞到的:`open_preview_onto` 对**页类**的分叉是「一个 tab 只有一张页」的单例规则,而那张页坐在一个**座位**上——`open_minted_page_on` 的第一条拒绝逐字写着「**Not a seat.** A torn-off pane has no leaf to put an engine on」。所以把一张 `.pdf` 卡抬成浮窗,得到的会是**一扇空窗**:页去了 tab 那个座位,窗里什么也没有。浮窗能装下一张页的唯一途径是把一个 pane **连着它的引擎一起撑出去**(`pop_out_preview`,§7.14b)——而把读者布局里的一个 pane 拆走,不是拖一张悬停卡所请求的事。

所以判据收成**一个** `file_peek_promotes()`,两条拒绝各是一个事实而不是一句含糊:**合成文档没有路径**(一次 commit 对文件的读法不在任何磁盘上,而仓库自己对它开窗的动词是另一个:`open_float_git_document`);**页类需要一个座位**(上一段)。不能抬的卡,它的头**什么都不改**:不扣住那次按下、不等六个像素、不给抓手光标,按下去就是昨天那扇门(`press_file_peek_door`)。**光标与手势读同一个谓词**,所以不会出现一只张开的手站在一个抬不起来的头上——那正是 §7.26 两天前在卡头上抓到的同一种病(一个画得出、按不动的动词),向外一层。页类向 `path_opens_as_a_page` 问(§7.16 ④ 那张唯一的表),红门 `one_predicate_says_whether_a_card_has_a_window_to_become` 同时盯住两件事:两个读者都走它,而它里面一个扩展名都不写。**将来浮窗能拥有自己的引擎时,删掉那一臂就完事,别处一行不动。**
