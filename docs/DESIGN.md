...
**明写的没做**:**没有去减少一帧要的字形总量**。真正让天花板变远的做法是「看不见的字不进图集」(`shape_chrome_labels` 至今没有 `shape_preview_body` 那道「整个盒子落在 clip 外就不排」的过滤,一个被完全遮住或滚出视野的标签照样铸它的位图),以及聚焦卡那一列按卡片尺寸重铸一整套迷你字号——两件都在这条账上,都留给专门的一片。本片做的是**不够分时怎么分**,不是**让它够分**。
