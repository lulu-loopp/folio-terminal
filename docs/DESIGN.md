...
**明写的没做**:**多页图集没做,因为量出来不该做**。这一片的触发条件是「①–④ 之后仍超过单张的 70%」,量到的是 0.8%;而多页(或按泳道分)会把④那条「一份字形多处用」拆掉,拿确定的损失换一个量不出来的收益。真要做的那天,先复现一帧**确实**顶到 roof 的实机 census,再动。第二件:**报告那次拒绝的真实成因仍未复现**——②说清了它不可能只是一帧的版面,但究竟是缩放动画那种「一帧里字号连续变化」,还是别的什么,这一片没有抓到;`BT_GLYPH_CENSUS` 就是为了下一次它出现时能被抓到而做的。
