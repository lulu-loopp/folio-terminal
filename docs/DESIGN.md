...
**③ 修法:一帧丢了字,给它一副新货架,而不是再看一眼旧的。** `GpuContext::close_the_frame` 现在是一帧对共享图集的**唯一**收尾动词,按这一帧自己的判决分两路:平常的一帧照旧 `trim`;**丢了字的一帧换一张新的 `TextAtlas`**——也就是一副从 256² 重新长起来的新货架。图集里的位图是**纯缓存**(每一张都只是一次 swash 调用),所以丢掉的只有再画一遍的时间;而那一帧本来就已经欠着、已经被 `PresentedWithoutText` 要着重来——**这才让重试真的是重试**。**它不是「定期清空」**:没有计时器也没有水位线,只在一帧**已经**丢了字的那一刻动手,那正是这份缓存被证明不再是缓存的唯一时刻。**而且一次拒绝只换一次**:`glyph_atlas_refitted` 这个闩没被一整帧的字清掉之前,第二次拒绝不再换——一副**上一帧才新做的**货架还是放不下,那是另一种病、另一个名字:这一帧要的东西比这台设备能做的最大纹理还大,重排货架治不了它,而每帧重排会把一扇只是装不下的窗口变成一扇每秒把全部字形重铸六十遍的窗口。闩由第一帧**完整落地**的帧清掉。安全性:换图集会让所有已 prepare 的坐标失效,所以它做在 `present_frame` 画完并 present 之后,而每条泳道的 draw 都由**这一帧自己的 prepare** 算出来的那个 flag 把着——这一帧没 prepare 的泳道不会被 issue;`TextRenderer` 的 pipeline 由共享的 `Cache` 按 format 铸,format 不变,所以渲染器本身仍然有效。
