...
一项改动的质量不由它修好了哪个样例来判定，而由它是否在所有情况下都维护了自己声称维护的那条规则来判定。本节约束所有编写工单、实现改动和审查改动的人。新工单从 `docs/templates/brief.md` 起笔，末轮审查从 `docs/templates/closure-review.md` 起笔——下面九条规则的可操作形式都在这两份模板里，不在这段文字里。协调方用模板写成的工单已经带着这些字段；实现方收到工单后发现有字段留空，先问协调方补齐，再动手。
