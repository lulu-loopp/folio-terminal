...
一项改动的质量不由它修好了哪个样例来判定，而由它是否在所有情况下都维护了自己声称维护的那条规则来判定。本节约束所有编写工单、实现改动和审查改动的人。新工单从 `docs/templates/brief.md` 起笔，末轮审查从 `docs/templates/closure-review.md` 起笔——下面八条规则的可操作形式都在这两份模板里，不在这段文字里。协调方用模板写成的工单已经带着这些字段；实现方收到工单后发现有字段留空，先问协调方补齐，再动手。
...
- **硬标准**（hard requirement）：无论工单内容如何，产品都不得违反的底线——丢失用户数据、崩溃、冻结窗口、显示与来源不符的内容、泄露隐私数据。
...
- 项目所有者（或工单中指名的协调方代理人）在动手之前签认了这些条件。
- 事后的任何改动都记录了改动人、批准人以及促成改动的反例，旧条件保留。

**为什么** —— 失败之后重新定义成功，是最低成本的"通过"方式。
