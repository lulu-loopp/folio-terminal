...
- **门**（door）：某一类副作用唯一具名的入口——读文件字节、起子进程、交给操作系统、起线程等，见 `docs/ARCHITECTURE.md` §6。经过门的副作用有一个答案回答"这件事在哪里发生"，并有一道守卫守住它。
...
- **改动改变事实归属时适用**：规则 11（先写设计说明、交 Codex 审）。不改变任何归属的功能工单不写设计说明。
- **有审查时适用**：规则 6（义务台账）。
...
### 规则 10 【预防】每张实现工单写明它对结构的影响：碰哪些有主的事实、走哪些门、增还是还哪笔结构债

**判据**
- 工单模板中的"结构影响"（Architecture impact）一节在起草时由协调方填写，验收时逐项核对。
- **碰到的事实**：改动写入、读取或改变其生命周期的每一项有主的事实——`docs/ARCHITECTURE.md` §4 的单一所有者表和五类所有权规则、`docs/plans/structural-debt.md` 与 2026-09-21 结构评审点名的所有权契约——每项写所有者（代码符号）和改动对它做什么：只读、写入、新增读者、改变生命周期。
- **经过的门**：改动经过的每一扇门——读文件字节、起子进程、交给操作系统、起线程、往 PTY 写字节——写明走哪扇门、哪条 lane。§6 表里还没有门的副作用，写出它今天唯一的入口函数；没有唯一入口，如实写"无门"。新增一种副作用，按 §6 的准入规则先有门。
...
> 案例：2026-09-21 的两份结构评审在代码里数出二十二项跨线程有不止一个所有者的事实，分属五类不同的问题（`docs/ARCHITECTURE.md` §4.2）；每一项都是某张工单顺手加上的，没有一张工单说过自己加了它。文件列的 `bt_app::files::read_directory` 读目录不经任何门、没有 lane、没有守卫，也没有人裁决过它该不该有（`docs/ARCHITECTURE.md` §6）——它落地时没有人问过它走哪扇门。

### 规则 11 【预防】改变事实归属的工单，先写一页设计说明交 Codex 审查，再实现

**判据**
- 工单把一项事实从一个所有者挪给另一个，或把它拆开（例如从按窗口改成按窗格），派出实现方之前先写一页设计说明，交 Codex 审查。
...
**为什么** —— 归属改变是结构评审排在最前的那一类问题（`docs/plans/structural-debt.md` D-1：会话状态没有独立于窗口的所有者），改错了付代价的是此后的每一张工单。一页纸的审查比一轮实现便宜。纯功能工单不走这一步，因为审查也有成本，不把仪式加在用不着它的地方。
