...
| 6 | `search::scan_history` / `scan_volatile` — the pattern re-run over every frozen line on every keystroke in the find box | `Runtime::turn`'s search refresh | **0.4.4** — observation lane |
...
| 13 | `sample_window_place` — 4 to 8 syscalls, at three call sites for one instant | `drain_pty`, `advance_strip_animation`, `FolioApp::user_event` | **0.4.4** — one sampling policy, not three resamplings |
| 14 | `Window::set_title` at five call sites with no throttle | `drain_pty`, `activate_tab`, `dress_new_window`, `finish_synchronized_update_if_due`, `finish_rename` | **0.4.4** — throttle it the way the IME caret area already is |
