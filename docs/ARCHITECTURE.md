...
`bt-install-channel` (ticket U-1), through the door. And one more after it: `update_trial::begin_watch`'s
...
| through the thread door | **26**, every one in `bt-app`; each is a `Worker` by its name and its body is lent a `WorkerCtx` (A1b) | `spawn_at_priority` |
...
|---|---|---|
| reading file bytes | `bt_platform::file_reads` — twelve named lanes (U-13 added `UpdateJournal`, the trial's watch), `Lane`, `Ledger::add`, the process-wide `LEDGER` | `file_reads_doors.txt` plus a source guard |
