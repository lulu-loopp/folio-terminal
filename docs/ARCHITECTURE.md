...
| constructing a child process | `bt_platform::quiet_command_named` (and `quiet_command`) — absolute path resolved by `handoff::program_on_path`; the macOS identity check's two system tools (`/usr/bin/codesign`, `/usr/sbin/spctl`) are named by their fixed absolute paths through `quiet_command`, bounded, in `bt_platform::macos_identity` (U-16) | pinned as the only `Command` construction |
