...
**当前状态**：M-1 第一部分（语料基建 + 双平面原型，三门 G1/G2/G3 全过）已过审；hygiene（任务 00）已交付并过审，**241 tests 全绿**，另有一次小返工（任务 00b）与你并行进行——它只碰 `bt-term` / `bt-transcript` / CI，**与四个 spike 无交集**。现在做 M-1 的剩余工作：**四个 spike**。做完才能进 M0。
