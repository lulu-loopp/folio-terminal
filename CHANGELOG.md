...
- **Dropping a file onto Folio now puts its path on the command line.** Drag a
  file out of File Explorer or the Finder and let go of it over a Folio window,
  and its path arrives in the terminal you are typing in, spelled for the shell
  running there — the same quoting a file you *copied* has had since 0.4.1, so
  PowerShell, `cmd`, a WSL shell and the rest each get the spelling they read.
  Several files let go of together arrive on one line, one argument each, and a
  name the shell has no way to spell is reported instead of being mangled.
  Nothing is run: the path is put in front of the cursor for you to finish the
  line.
